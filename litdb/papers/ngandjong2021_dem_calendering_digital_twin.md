...
### (c) ★★ **압력축은 닫혀 있다 — 그 마지막 후보를 열었고, 거기에는 축이 없다** (2026-09-11 추가)

위 문단은 *"압력축이 정말 필요하면 남은 후보는 **ref [33]/[36] Xu 2023 (JPS 554, 232294)** 이고,
그것은 아직 정본에 없다"* 로 닫혀 있었다.  **그 후보를 digest 했다** →
**`papers/xu2023_realistic_am_shape_cgmd_calendering.md`** (2026-09-11).  **판정:**

> ⛔ **후속편은 이 축을 밀지 않았다 — 없앴다.**  Xu 2023 본문·SI 전수 grep 에서 `MPa` **0 회**.
> 논문 전체에서 `Pa` 단위로 적힌 수는 **슬러리 NPT 배로스탯 10⁵ Pa (= 1 atm)** 하나뿐이고 `GPa` 는
> 전부 탄성계수다.  압연 실험 조건은 *"various applied pressures"* 라고만 적힌다.
> 공정 축은 **compression degree (CD) 0 – 42.2 %** 하나다.
>
> ★★ **그리고 잴 수 있었다** — Xu 2023 의 압연 실험은 **같은 장비(BPN250, People & Technology),
> 같은 라인속도(0.54 m/min), 같은 롤온도(60 °C)** 다.  즉 **이 논문이 FlexiForce 힘센서 필름으로
> 갭 → 압력을 환산한 그 리그 그대로**인데 압력을 보고하지 않았다.  *"못 했다"가 아니라 "안 했다".*
>
> ⛔ **게다가 후속편은 이 카드가 줄 수 있었던 *floor 판정*마저 못 준다**: Xu 의 ε(CD) 곡선은
> 마지막 구간에서 **dε/dCD ≈ −0.90 %p/%**(96 % AM) · **−0.96**(94 % AM) 로 **가파른 채 끝난다**
> (`derived(ours)`) ⇒ 도달한 최저 **20.4 % / 25.0 %** 는 **바닥이 아니라 마지막 점**이다.
> 이 카드의 *"86 → 156 MPa 에서 Δε = 0.1 %p = 진짜 포화"* 는 **계보에서 유일한 포화 판정**으로 남는다.
>
> ⛔ **이 카드의 P↔ε 를 빌려 Xu 의 CD 에 MPa 를 붙이는 것도 안 된다**: 같은 96:2:2 조성인데
> **압연 전 실측 porosity 가 42.15 %(이 논문, stated) vs ≈47.5 %(Xu, digitized) = 5.4 %p 차** ⇒
> **코팅·건조가 다른 침대**다.  같은 압연기라는 사실은 침대가 같다는 뜻이 아니다.
> ★ 조건부 최대치: Xu 의 실험 최대 CD 30.9 % 에서 ε 25.1 % 는 **이 논문이 86–156 MPa 에서 도달한
> 26.2 %** 와 같은 이웃이다 ⇒ *"Xu 의 압연은 대략 이 축의 끝(≲156 MPa)에 있고 넘지 않는다"* 가
> 말할 수 있는 최대치이고, 그것도 **성립하지 않는 가정(같은 침대) 위**에 있다.
>
> ⇒ ★★ **계보 5 편(이 논문 + Xu 2023 + DL 3 편) 중 MPa 축을 가진 것은 이 논문 하나이고, 그것으로 확정이다.**
> 우리 Heckel knee **P_y = 138 MPa** 와 생산점 **300 MPa** 로 옮길 수 있는 **압력 값은 여전히 0 개**다.
> §3-B-3 의 한 문단 결론은 **수정 없이 유지**되고, 이제 *"남은 후보"* 라는 단서만 지워진다.

★ **그 대신 Xu 2023 이 새로 여는 축 세 개** (이 카드에는 없는 것들):
1. **형상 관측량** — Krumbein 구형도 `(bc/a²)^⅓` 분포.  두께 36–42 % 감소에 **모드 −0.04 ~ −0.06** 만 이동
   ⇒ **우리 AM-frozen scaffold 규약의 외부 지지**(AM 을 풀어 준 모델이 *"modest deformation"* 이라 결론).
2. **구 근사의 대가** — 같은 조성·같은 porosity 에서 **실형상 τ / 구 근사 τ = 1.55 ~ 2.40×** 이고
   **압밀할수록 벌어진다** ⇒ 이 카드의 구-근사 τ 값들이 **하한**이라는 뜻이다.
3. **CD 축의 연장** — 이 논문의 실험 최대는 ε 42.15 → 26.23 % ⇒ **CD ≈ 21.6 %**(`derived(ours)`),
   Xu 는 **sim 42.2 % · exp 39.7 %**.  그 축은 **변위 제어 + 해제·완화**라 우리 MPM `hold`/`release` 와
   **같은 종류**다 ⇒ *"모델 ↔ 모델"* 비교에는 오히려 더 쓸 수 있다.

⚠ **한 가지 되돌아오는 경고**: Xu 2023 의 **ν_CBD 가 0.3 으로 바뀌었다**(이 카드 §C1 이 정정한 바로 그
*"CBD 만 0.5 = 비압축"* 설정이 사라졌다).  후속편은 **그 변경을 언급도 정당화도 하지 않는다**.
⇒ *"CBD = 부드럽지만 비압축"* 을 이 계보의 합의로 인용하지 말 것 — **같은 그룹 안에서 2 년 만에 뒤집혔다.**
