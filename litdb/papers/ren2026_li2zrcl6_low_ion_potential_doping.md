...
> 본문 PDF `80a8e6ce` (Authorea 표지 2 pp + 원고 29 pp = 31 pp) + **SI `205249ff`.docx**(Fig S1–S20 캡션 · Table S1–S6 · eq 1–5 · ref 1–19) 전문 정독 · digested `2026-07-28` · **2차 패스(본문 그림 픽셀 독립 검증) `2026-08-04` → §20** · **3차 패스(우리 Nd 캠페인 조준 재독) `2026-09-09` → §21** · status ✅
>
> 🔁 **재투입 이력**: 2026-09-09 에 같은 논문이 `101.` 번호로 다시 inbox 에 들어왔다(업로드 `205eaa0a`, 31 pp, DOI·본문·Methods 전부 동일 = **같은 v1**). 새 slug 를 만들지 않고 **이 문서에 §21 을 붙였다.** 재투입 사유는 **우리 `ndo_lpscl16`(Nd·O 공도핑 Li₅.₄PS₄.₄Cl₁.₆) 캠페인** — Nd³⁺ 를 고체전해질에 넣은 *계산* 선례가 드물어서 **수치가 아니라 "Nd 를 어떻게 다뤘는가"(자리·전하보상·4f·해석)** 를 뽑으려는 것이다. ⚠ 이번 재투입에는 **SI 가 없다**(본문 PDF 만) — §21 의 SI 인용은 전부 1차 패스 기록을 재사용한 것이고, 새로 검증한 것이 아니다.
>
...
| ⑨ ★★★ **우리 Nd 캠페인이 이 논문에서 가져갈 것 / 못 가져갈 것** (2026-09-09 추가) | **§21** ★★★ | ★1 자리·전하보상 = **Zr 자리 치환 + 화학식에 Li 를 더해 중성 셀로 보상**(배경전하·공공 없음), 단 **보상 Li 를 어느 자리에 넣었는지는 논문 어디에도 없다**(§21.1). ★4 **Nd–Cl 결합길이·배위수는 논문에 하나도 없다**(§21.4). ★5 **4f = DFT+U 인데 U 값·스핀·점유행렬 정책 전부 미기재** → 그들의 조성 선택(E_coh)이 **정의되지 않은 보고량** 위에 서 있다(§21.5). **우리 frozen-4f 는 이 실패모드를 회피한다 — 이 축에서는 우리 규약이 더 방어 가능하다**(§21.9). 신규 비판 4건: 서술자의 **내부 반례가 Nd 자신**(§21.2) · VDOS 3패널 **정규화 불가 + Cl 밴드끝 불변**(§21.3) · **질량효과 미분리**(§21.3) · **matched-x 에서 Er↔Nd 순위가 뒤집힌다**(§21.6) |
...
| **2a** | pristine LZC + Er0.5 + Nd0.375 결정구조 + [ZrCl₆]/[ErCl₆]/[LiCl₆]/[NdCl₆] 다면체 갤러리 | 다면체 단위 카탈로그 제시 양식. **3차 패스(§21.1) 판독**: 네 다면체가 전부 **8면체**로 그려져 있다 = 도펀트는 **Zr 8면체 자리**, **Li 도 8면체 자리**. 도핑 모델은 **층상 규칙배열**(RE 8면체가 특정 층·열에만) — 무작위/SQS 가 아니다. **보상 Li 는 별도로 표시돼 있지 않다** |
...
| **4c–4e** | ★ **원소분해 VDOS** — LZC(0–10 THz) / Er0.5(0–14) / Nd0.375(0–14), 범례 VDOS·Cl·Li·Zr(+Er/Nd) | ★ **우리 공백 축의 실물 예시**. ⛔ **y 스케일이 패널마다 다르고(≈10/50/50) x 범위도 다르다** → 패널 간 높이 비교 무효(§7.3). **3차 패스(§21.3) 고해상 재판독 3건**: ⛔ 셀 원자수는 72 vs 76/75(모드 수 차 5.6 %)인데 **peak 가 4.6배** → 정규화가 패널마다 다르다(물리 불가) · ⛔ **Cl 밴드 상단(≈5.0–5.1 arb.)이 세 패널에서 안 움직인다** = 골격 연화의 직접 증거가 아니다 · ⛔ **x축에 단위가 인쇄돼 있지 않다**(본문만 "THz") |
...
## 21. ★★★ 3차 패스 — **우리 Nd 캠페인 조준 재독** (2026-09-09)

### 21.0 왜 다시 읽었나 · 이번에 실제로 본 것

**트리거.** 우리가 지금 `ndo_lpscl16` = **Nd·O 공도핑 Li₅.₄PS₄.₄Cl₁.₆(argyrodite)** 을 돌리고 있다.
2026-09-09 현재 하는 일은 **O 배열 3종 × 셀 2종 = 6점의 DFT 총에너지로 배열 하나를 고르는 것**이고
(보고량 카드 `db/properties/ndo_lpscl16_o_motif_estimand_2026_09_09.json`),
그 어닐본에서 **O 가 Nd 와 2.19–2.26 Å 로 배위하는** 배열이 나왔다.
**Nd³⁺ 를 고체전해질에 넣은 *계산* 문헌이 드물어서** 이 논문을 다시 부른 것이다.

⛔ **목적은 수치 이식이 아니다.** 계가 다르다(할라이드 Li₂ZrCl₆ ↔ 황화물 argyrodite).
가져오는 것은 **"Nd 를 어떻게 다뤘는가"** — 자리 · 전하보상 · 반경 · 4f · 해석 — 이고,
가져오지 않는 것은 **σ · Eₐ · D · 장벽 · 격자상수 전부**(§⛔⛔ 최우선 규율 2·3 그대로).

**이번 패스에서 실제로 본 그림 (본 것과 안 본 것을 구분한다).**

| 본 것 | 어디서 | 왜 |
|---|---|---|
| ✅ **Fig 1** (설계 도식) | `litdb/figures/…/fig_1.png` | 기구 서사의 그림 문법 확인 |
| ✅ **Fig 2** 전 패널 + **Fig 2a 600 dpi 확대** | 크롭 PNG + p.11 좌표 직접 렌더 | ★1 **도펀트 자리·보상 Li 표시 여부** |
| ✅ **Fig 4** 전 패널 + **Fig 4c–4e 500 dpi 확대** | 크롭 PNG + p.19 좌표 직접 렌더 | ★3 **lattice dynamics 의 실제 증거력** |
| ⛔ **안 봤다: Fig 3, Fig 5** | — | 2차 패스(§20)가 이미 **픽셀 단위로** 전수 검증했고(`tools/litdb/ren2026_fig_verify.py`), 이번 질문(Nd 자리·4f·결합길이)에 걸리지 않는다. §21 의 Fig 3/5 인용은 **전부 §20 기록의 재사용**이다 |
| ⛔ **안 봤다: SI Fig S1–S20** | — | **이번 재투입에 SI 가 없다**(본문 PDF 만). §21 의 Table S1–S6 인용은 **1차 패스 기록의 재사용**이고 이번에 재검증하지 않았다 |

**크롭 정합성 확인**: `figures.json` 의 5장(fig_1…fig_5)은 이번 업로드본과 쪽·bbox 가 그대로 맞는다.
⚠ 단 **`fig_1.png` 맨 위에 앞 문단 꼬리("after 100 cycles at 0.5 C.")가 한 줄 딸려 들어와 있다** — 캡션 오배정은 아니고 bbox 상단 여유다. 그림 내용은 캡션과 일치한다.

---

### 21.1 ★1 **Nd(·Er)를 어느 자리에, 어떻게 넣었나 — 그리고 전하보상**

#### (a) 치환 자리 = **Zr 자리** (Li 자리 아님). 근거 3중.

1. **화학식** `Li₂₊ₓZr₁₋ₓMₓCl₆` — Zr 계수가 x 만큼 줄고 M 이 x 만큼 는다. Li 자리는 건드리지 않는다.
2. **Fig 2a 다면체 갤러리** (600 dpi 재판독): `[ZrCl₆]` · `[ErCl₆]` · `[LiCl₆]` · `[NdCl₆]` **네 개가 전부 8면체**로 그려져 있다.
   Li₃YCl₆형 `P3̄m1` 골격에서 **금속도 Li 도 Cl 8면체 자리**이고, RE 는 **Zr 이 있던 8면체**를 그대로 받는다.
3. **본문 §2.1**: *"substitution of smaller Zr⁴⁺ (72 pm) by larger Er³⁺ (89 pm) and Nd³⁺ (98 pm), which induces local **[MCl₆] octahedral expansion**"* — 명시적으로 M 8면체다.

#### (b) 전하보상 = **화학식에 Li 를 더해 중성 셀로**. 배경전하·공공 없음.

Zr⁴⁺ → M³⁺ 는 **양전하 −1/치환**이다. 논문의 처리는 한 줄이다:

> *"The aliovalent substitution of Zr⁴⁺ by Er³⁺/Nd³⁺ **introduces additional Li⁺ through charge compensation** and increases the carrier concentration."* (§1)

**우리 1차 패스가 이미 독립 검산으로 확정한 것**(§6.3): AIMD 셀은 `2×2×2 = Li₁₆Zr₈Cl₄₈`(72 원자)이고
**N_Li = 8(2+x)** 를 넣어야 SI eq(5) 로 Table S5 의 σ 11/11 이 **0.03–0.24 % 안에서 재현**된다. 즉:

| x | Li | Zr | M(Er/Nd) | Cl | nat | 보상 Li |
|---:|---:|---:|---:|---:|---:|---|
| 0 | 16 | 8 | 0 | 48 | 72 | — |
| 0.125 | 17 | 7 | 1 | 48 | 73 | **+1** |
| 0.25 | 18 | 6 | 2 | 48 | 74 | **+2** |
| 0.375 | 19 | 5 | 3 | 48 | 75 | **+3** |
| 0.5 | 20 | 4 | 4 | 48 | 76 | **+4** |
| 0.625 | 21 | 3 | 5 | 48 | 77 | **+5** |

> ✅ **확정: 도펀트 1개당 Li 원자 1개를 셀에 실제로 더 넣었다. 중성 셀이다.**
> **배경전하(jellium) 없음 · Li 공공 없음 · 침입형이라는 표현 없음 · 결함형성에너지·전하보정(Makov–Payne/FNV) 없음.**
> 우리 계에서 정확히 같은 종류의 처리이고(§21.9), **이 논문은 그 처리의 *외부 선례*로 쓸 수 있다.**

#### (c) ⛔ 그런데 **보상 Li 를 *어느 자리*에 넣었는지는 논문 어디에도 없다.**

찾은 서술의 전부:

> *"stable doped configurations are obtained through **structural optimization**"* (§4 Methods)
> *"the additional Li⁺ introduced by aliovalent substitution helps **increase Li⁺ site occupancy and migration-pathway connectivity**"* (§2.3)

없는 것:
- **보상 Li 의 Wyckoff 자리 / 8면체인지 4면체인지 / 부분점유 처리** — 없음.
- **배열 열거·SQS·Ewald 정렬·앙상블 평균** — 없음. **조성당 배열 1개.**
- **배열 선택 규칙** — 없음("구조최적화로 안정한 것을 얻었다"가 전부).
- **Rietveld 로 실험 점유율을 확인한 표** — 없음(§5.3: 정련 좌표·점유율 표가 본문·SI 어디에도 없다).

간접 단서는 **Fig 2a 뿐**이다: `[LiCl₆]` 8면체만 그려져 있으므로 평형 Li 는 8면체,
그리고 NEB 서술이 *"the originally **high-energy tetrahedral sites** become stabilized as intermediate metastable states"* 라 하므로
**4면체는 통과점이지 정착지가 아니다** → 보상 Li 는 **빈 8면체 자리**로 갔다고 읽는 것이 자연스럽다. **다만 이것은 우리 추론이고 논문의 진술이 아니다.**

> ⛔⛔ **이것이 이 3차 패스의 1번 소득이자 1번 경고다.**
> 저자들은 **aliovalent 보상 자리 선택이라는, 결과를 좌우하는 자유도를 통째로 문서화하지 않았다.**
> 그 대가가 §21.6 에서 수치로 드러난다 — **같은 x 에서 Er↔Nd 순위가 뒤집히고, 한 원소 안에서 σ 가 35배 흔들린다.**

---

### 21.2 ★2 **"low ion potential" 이 정확히 무엇인가 — 그리고 사전 선언이었나**

#### (a) 정의식 (1차 패스 §4.1–4.2 확정, 재확인)

$$\Phi = \frac{Z}{r}$$

> *"the **ionic potential (Φ = Z/r**, where Z is the cation valence state and r is the ionic radius in the corresponding coordination environment)"* (§1)

- **Z** = 양이온 **형식전하**(Bader 아님 — Bader/COHP 는 이 논문에 **0건**).
- **r** = **Shannon 6배위 이온반경**. 본문이 Zr⁴⁺ 72 / Er³⁺ 89 / Nd³⁺ 98 pm 을 직접 쓴다.
- **단위**: 논문 안에서 **세 군데가 서로 다르다** — Table S6 은 `pm⁻¹`(0.0556 …), `Fig. 4b` 우축은 `Å⁻¹`, 같은 그림 범례는 `10⁻¹/Å⁻¹`, `Fig. 4a` y축은 `nm⁻¹`. **값은 하나, 라벨이 셋이다.**
- 1차 패스에서 **Table S6 18계 전값을 Shannon 반경으로 18/18 역산 일치** 확인 → 정의식은 문자 그대로다(§4.2).

| 이온 | Shannon r (Å) | Φ (Å⁻¹) | Φ (pm⁻¹, 그들 표 단위) |
|---|---:|---:|---:|
| Zr⁴⁺ (VI) | 0.72 | 5.556 | 0.0556 |
| **Er³⁺ (VI)** | 0.890 | **3.371** | **0.0337** |
| **Nd³⁺ (VI)** | 0.983 | **3.052** | **0.0305** |
| (참고) **P⁵⁺ (IV) = 우리 계** | **0.17** | **29.41** | **0.294** |

#### (b) 선정 기준은 사전 선언인가 — **부분적으로만.**

**선언된 것**: *"Through a comparative screening of **18 reported dopant elements**, Er and Nd are selected as representative doping candidates because they exhibit **low ionic potentials and good size compatibility** with the LZC lattice."* (§1)

**선언되지 않은 것 — 그리고 이게 결정적이다**:
- **컷 값이 없다.** "low" 가 얼마 이하인지 숫자로 정하지 않았다. 18계 Φ 범위는 Ca 0.0200 – Mo 0.1017 pm⁻¹ 인데, Er(0.0337)·Nd(0.0305) 보다 **더 낮은 계가 넷이나 있다**(Ca 0.0200 · Zn 0.0270 · Cu 0.0274 · Mg 0.0278). "가장 낮은 것"을 고른 게 아니다.
- **"size compatibility"의 정의가 없다.** 어떤 지표로 잰 적합도인지 없음. Er·Nd 는 오히려 **Zr 보다 24 %·36 % 큰** 이온이다.
- **희토류를 왜 골랐는지의 진짜 이유(가용성·선행연구 [33] = 저자 자신의 논문)가 서술자 논리와 분리되지 않는다.**
- **Fig. 4b 의 스크리닝 그림에서 자기 최적 조성이 3위다** — Sc₀.₄ 1.50 · Ta₀.₃ 1.42 > Er₀.₅ 1.32(§20.6). **자기 스크린이 자기 선택을 지지하지 않는다.**

#### (c) ★ 신규 — **서술자의 가장 깨끗한 반례가 이 논문의 Nd 자신이다**

1차 패스 §4.5 의 반례 3건(Ta/Nb 동률 · Zr/Al 동률 · 저-Φ 끝 붕괴)은 **전부 남의 논문 15편을 가로지른 비교**다
(합성법·펠릿밀도·측정온도·total vs bulk 가 전부 다르다 → 반례로서 약하다).
**그런데 Er vs Nd 는 같은 호스트·같은 볼밀 레시피·같은 계산 규약의 *내부 대조*다.** 그 결과:

| | Φ (pm⁻¹) | **Φ 로 본 예측** | NEB Oct–Tet | NEB Oct–Oct | σ_AIMD(최적) | σ_exp(최적) |
|---|---:|---|---:|---:|---:|---:|
| **Er³⁺** | 0.0337 (더 **높다**) | 더 나쁠 것 | **0.370 eV** | 0.514 eV | **4.00 mS/cm** | **1.32 mS/cm** |
| **Nd³⁺** | 0.0305 (더 **낮다**) | 더 좋을 것 | 0.417 eV | 0.561 eV | 3.56 mS/cm | 1.13 mS/cm |
| | | | ❌ **뒤집힘** | ❌ **뒤집힘** | ❌ **뒤집힘** | ❌ **뒤집힘** |

> 🔑🔑 **판정: 논문이 스스로 통제해 수행한 유일한 2원소 비교에서, 네 개의 측정축 전부가 서술자의 예측과 반대로 나온다.**
> 그리고 **Er 이 이긴 조성(x=0.5)은 Li 가 20개, Nd 가 진 조성(x=0.375)은 19개** — 즉 **Li 캐리어 수가 더 많은 쪽이 이겼다.**
> 이는 1차 패스가 Table S6 18계에서 얻은 결론(**Li 함량 ρ = +0.628 ≫ Φ ρ = −0.089**)과 정확히 같은 방향이다.
> ⇒ **"저-Φ" 는 서사이고, 실제로 움직이는 변수는 aliovalent 치환이 끌고 들어온 Li 개수다.**
> ⛔ 이 논문을 근거로 **"Nd 는 Φ 가 낮아서 좋다"** 를 우리 문서 어디에도 쓰지 않는다.

---

### 21.3 ★3 **"lattice dynamics 를 조절한다" 를 무엇으로 쟀나**

#### (a) 잰 것 / 안 잰 것

| 양 | 있나 | 값 |
|---|---|---|
| **원소분해 VDOS (phonon DOS)** | ✅ `Fig. 4c`–`4e` 3계 | **그림만. 수치 0** |
| **⟨ω⟩ (VDOS 1차 모멘트)** | ⚠ **정의만** | Methods 에 식을 적어 놓고 **값을 본문·표·SI 어디에도 안 냈다** |
| phonon **분산곡선** | ⛔ | Methods 는 "extract the phonon dispersion relations" 라 하는데 그림이 없다 |
| **허수모드 점검**(동적 안정성) | ⛔ **없음** | 준안정 도핑 구조를 다루면서 안정성 진술 0 |
| Debye 온도 · 열용량 · Grüneisen · 음속 | ⛔ **전부 없음** | |
| **탄성상수 C_ij / 체적탄성률** | ⛔ **없음** | "격자 강성"을 말하면서 강성을 안 쟀다 |
| Raman 이동량 (cm⁻¹) | ⛔ | "broadening + slight redshift" 정성 서술만 |

**즉 "lattice dynamics 조절"의 정량 근거는 `Fig. 4c`–`4e` 세 장의 육안 비교가 전부다.**

#### (b) ★★ 신규 — **그 세 장은 서로 비교할 수 없는 그림이다** (500 dpi 재판독)

`Fig. 4c`–`4e` 를 확대해 실제로 봤다. 세 가지가 나왔다.

**① 정규화가 패널마다 다르다 — 물리적으로 불가능한 조합이다.**

| 패널 | 계 | nat | 모드 수 3N | x 범위 (arb.) | 총 VDOS 최대 (figure-read ≈) |
|---|---|---:|---:|---|---:|
| `Fig. 4c` | Li₂ZrCl₆ | **72** | 216 | 0 – **10.8** | **≈ 9.7** |
| `Fig. 4d` | Er0.5 | **76** | 228 | 0 – **13.9** | **≈ 45** |
| `Fig. 4e` | Nd0.375 | **75** | 225 | 0 – **13.9** | **≈ 41** |

> ⛔⛔ **모드 수는 5.6 % 밖에 안 다른데 곡선 높이는 4.2–4.6배 다르고, 게다가 지지구간(x 범위)은 도핑계가 *더 넓다*.**
> ∫g(ω)dω = 3N 이 고정이면 **더 넓게 퍼진 쪽이 더 낮아야** 한다. 정반대다.
> ⇒ **세 패널은 서로 다른 임의 정규화(a.u.)로 그려졌다.**
> **그런데 논문의 핵심 주장 — *"the Er/Nd-doped systems exhibit **enhanced** phonon density of states in the low-frequency region, especially at approximately 1–2 THz"* — 은 오로지 이 패널 간 높이 비교로만 만들어졌다.**
>
> **자기 정규화로 다시 보면 그 주장이 사라진다** (figure-read ≈, 판독오차 ±5 %p):
> 1–2 (arb.) 구간에서 각 패널 자신의 최대값 대비 총 VDOS 는 **4c ≈ 46 % · 4d ≈ 44 % · 4e ≈ 44 %** — **사실상 같다.**

**② Cl 밴드의 상단이 세 패널에서 안 움직인다.**
Cl(청록) 성분이 끝나고 VDOS 가 0 근처로 떨어지는 **밴드 갭 위치가 세 패널 모두 ≈ 5.0–5.1 (arb.)** 이다
(4c 는 ≈5.0–5.6 의 넓은 갭, 4d/4e 는 ≈5.1–5.3 의 좁은 갭). **figure-read ≈.**
> ⛔ **음이온 골격이 실제로 연화됐다면 Cl 밴드 상단이 내려가야 한다. 안 내려간다.**
> 5 (arb.) 위쪽에서 늘어난 것은 **Li(주황) 성분**인데, 그건 도핑으로 **Li 를 16→19/20 개로 늘렸으니 당연하다**(§21.1b).
> ⇒ **그들 자신의 VDOS 가 보여주는 가장 큰 변화는 "Cl 골격 연화"가 아니라 "Li 부분스펙트럼의 증가"다.**
> 결정적 검사(= **Cl 원자당으로 정규화한 Cl-투영 VDOS 의 1차 모멘트 비교**)는 그들 데이터로 가능한데 **하지 않았다.**

**③ 도핑계에만 11.5–13.9 (arb.) 에 고립된 뾰족선이 여러 개 있다.**
pristine 에는 없다. 밴드에서 떨어져 나온 델타형 선은 (i) 국소모드이거나 (ii) 수치 인공물인데,
**Methods 가 유한변위(0.01 Å)와 DFPT 를 동시에 적어 놓아**(§7.1 모순) 어느 쪽인지 판정할 수 없고,
**허수모드 점검이 없어서** 계산의 건전성 자체가 확인되지 않는다.

**④ x 축에 단위가 인쇄돼 있지 않다.** 세 패널 모두 그냥 `Frequency`. THz 라는 근거는 본문 문장 하나뿐이다.

#### (c) ★★ 신규 — **질량 효과를 분리하지 않았다** (계산은 산수다)

포논 진동수는 $\omega \propto \sqrt{k/m}$ 다. 논문은 **k(결합 세기)가 약해져서** 저주파가 늘었다고 주장하는데,
**Zr → Er/Nd 치환은 m 을 크게 바꾼다.**

| | m (u) | M-투영 모드 ($\omega\propto m_M^{-1/2}$) | M–Cl 신축 (환산질량) |
|---|---:|---:|---:|
| Zr | 91.224 | 기준 | 기준 |
| **Er** | 167.259 | **×0.739 (−26.1 %)** | ×0.934 (−6.6 %) |
| **Nd** | 144.242 | **×0.795 (−20.5 %)** | ×0.947 (−5.3 %) |

> ⛔ **즉 결합 세기 k 가 조금도 안 변해도, 무거운 희토류를 넣는 것만으로 M 중심 모드는 20–26 % 내려간다.**
> 논문은 이 항을 **한 번도 언급하지 않고** 저주파 증가를 전부 *"weakened M–Cl electrostatic interaction"* 으로 귀속한다.
> 두 원인을 가르는 방법은 표준이고 그들 계산으로 가능했다 — **힘상수 행렬 자체를 비교**하거나, **질량만 바꾼 가상 동위원소 계산**을 하거나, **Cl-투영 VDOS 를 Cl 원자당으로** 보면 된다. 셋 다 안 했다.
> 🔑 **우리 계에서는 이 항이 훨씬 크다: P → Nd 는 m 이 30.97 → 144.24 로 4.66배, ω 는 ×0.463 (−53.7 %)** (§21.9-④).

#### (d) 그래서 "이온전도와 어떻게 연결된다" 는 상관인가 인과인가

논문의 사슬은 **저-Φ → M–Cl 약화 → Cl 골격 연화 → 저주파 모드↑ → 이동 중 골격이 협동적으로 반응 → 장벽↓ → σ↑** 다.
이 사슬에서 **실제로 측정된 고리는 하나도 없다**:

| 고리 | 측정됐나 |
|---|---|
| 저-Φ → M–Cl 약화 | ⛔ **Bader·COHP·ICOHP·전하밀도 0건.** 증거는 XPS "slight shift"(이동량 수치 없음)뿐 |
| M–Cl 약화 → 골격 연화 | ⛔ 힘상수·탄성·⟨ω⟩ 값 0. 위 (b) 로 VDOS 비교 자체가 무효 |
| 연화 → 장벽↓ | ⛔ **연화 지표와 장벽의 상관 그림이 없다.** NEB 3점뿐 |
| 장벽↓ → σ↑ | ⚠ AIMD 로 연결하지만 **prefactor 를 한 번도 논하지 않는다** — 그런데 그들 자신의 데이터가 **Meyer–Neldel 보상선(R² 0.93)** 위에 있다(§6.7) |

> ⛔ **판정: 인과 사슬로 서술돼 있으나 실증된 것은 "치환하면 값들이 같이 움직인다"는 동반변화뿐이다.**
> `[Kraft]` 2017(우리 물질 Li₆PS₅X, 음속·Debye 진동수·RUS 실측)은 **연화가 Eₐ 와 σ₀ 를 동시에 내려 상쇄된다**는 것을 보였다 — 이 논문은 그 상쇄를 보지 못한다(§7.5).

---

### 21.4 ★4 **Nd–음이온 결합길이 · 배위수**

#### (a) 논문에 있는 것 — **거리 값은 하나도 없다.**

본문 · Methods · 그림 · SI 캡션 · Table S1–S6 전수에서 **Nd–Cl(또는 Er–Cl, Zr–Cl) 결합길이·배위수 수치는 0건이다.**
있는 것은 **이온반경 3개**(Zr⁴⁺ 72 / Er³⁺ 89 / Nd³⁺ 98 pm)와 **정성 서술**뿐:

> *"the larger ionic radii of Er³⁺ and Nd³⁺ induce **M–Cl bond-length expansion** and local lattice expansion"* (§1)
> *"induces local **[MCl₆] octahedral expansion** and unit-cell expansion"* (§2.1)

배위수는 **그림으로만** 주어진다 — `Fig. 2a` 의 `[NdCl₆]`·`[ErCl₆]` **8면체 = CN 6**.
정련된 원자좌표·점유율 표가 없으므로(§5.3) **실험에서 결합길이를 되뽑을 수도 없다.**

> ⛔ **★4 의 답: 논문에 없다.** Nd–Cl 도, Nd–O 도(애초에 이 계에 O 가 없다), Nd–S 도 없다.
> ⇒ **우리 어닐본의 Nd–O 2.19–2.26 Å 와 *직접* 대조할 문헌값이 이 논문에는 존재하지 않는다.**

#### (b) 그래서 남는 유일한 대조축 = **Shannon 이온반경 합** (우리 산수, 논문 값 아님)

| 쌍 | Shannon 반경 합 (Å) | 비고 |
|---|---:|---|
| Zr⁴⁺–Cl⁻ | 2.53 | 논문의 pristine 기준 |
| Er³⁺–Cl⁻ | 2.70 | +0.17 Å |
| **Nd³⁺–Cl⁻** | **2.79** | **+0.26 Å (+10.3 %) — 이것이 논문이 "M–Cl 팽창"이라 부르는 것의 크기 추정치** |
| **Nd³⁺–S²⁻** | **2.82** | 우리 계 |
| **Nd³⁺–O²⁻** | **2.38** | 우리 계 |

#### (c) 우리 쪽 실측 — **이 digest 에서 직접 쟀다** (문헌값 아님, 우리 계산값)

`db/structures/ndo_lpscl16_rietveld_2026_09_07/anneal_2026_09_07/*_post_relax.xyz` **6구조 전수**,
ASE `get_all_distances(mic=True)`, Nd 2개/셀:

| 쌍 | 우리 실측 범위 (Å) | Shannon 합 | 차 |
|---|---|---:|---|
| **Nd–O** | **2.189 – 2.377** (배위 O 2–3개) | 2.383 | **−0.19 ~ −0.01 Å (짧다)** |
| **Nd–S** | 2.670 – 2.936 | 2.823 | −0.15 ~ +0.11 (걸친다) |
| **Nd–Cl** | 2.836 – 3.389 | 2.793 | **+0.04 ~ +0.60 (길다)** |

세부: 셀마다 **Nd 두 개의 환경이 갈린다** — 하나는 **S 4–6개(2.67–2.94 Å) + Cl 1–2개**의 황/염소 케이지,
다른 하나는 **O 2–3개(2.19–2.38 Å) + S** 의 산소 케이지. `O-distributed` 배열에서는 두 Nd 다 O 와 안 붙는다(최근접 O 3.12 / 10.5 Å).

> 🔑🔑 **이 대조에서 나오는 판정 하나** — Nd 는 우리 계에서 **O 를 가장 짧게, Cl 을 가장 길게** 잡는다.
> 즉 **국소적으로 Nd–O 는 *조이는* 모티프**다.
> **이 논문의 서사(저-Φ RE = 음이온 골격을 *무르게* 한다)를 우리 Nd–O 계에 그대로 옮기면 부호가 반대일 수 있다.**
> ⛔ 그러므로 원고·덱에 *"Nd 가 격자를 유연하게 한다"* 를 이 논문 근거로 쓰지 않는다.
> (⚠ 위 우리 값은 **UMA 어닐본 기하**다 — DFT 이완 기하가 아니고, 아직 어떤 db 표에도 등록되지 않았다. 인용 시 그대로 밝힌다.)

---

### 21.5 ★5 **계산 설정 · Nd 의 4f 처리** — 이 절이 ★ 중에 제일 중요하다

#### (a) 설정 전수 (§6.1 재게, 이번 본문에서 재확인)

| 항목 | 값 | 미기재 |
|---|---|---|
| code | **VASP** | 버전 |
| pseudo | **PAW** [36] | 어떤 POTCAR(예: `Nd_3`, `Er_3` 인지) **미기재** |
| functional | **GGA-PBE** [37] | **vdW 보정 언급 없음** |
| **4f 처리** | *"To accurately describe the strong correlation effect of 4f electrons in the rare-earth elements Er³⁺ and Nd³⁺, the **DFT+U** correction method is applied to the **4f orbitals** of Er and Nd."* | ⛔ **U 값 · J · Dudarev/Liechtenstein · 스핀분극(ISPIN) · 자기배열 · NUPDOWN · 점유행렬 초기화·구속(`OCCEXT`/occupation-matrix control) 전부 없음** |
| ecut | **520 eV** | |
| k-mesh (정적) | **3×3×3** | 셀 크기 대비 기준 없음 |
| 수렴 | 전자 1.0×10⁻⁴ eV / 이온 1.0×10⁻³ eV / 힘 **0.02 eV Å⁻¹** | |
| 셀 | **2×2×2 = 72–77 원자**(§21.1b) | 셀 형상·격자 이완 여부(ISIF) 미기재 |
| NEB | Henkelman–Jónsson [39] | **이미지 수 · CI-NEB 여부 · 스프링상수 미기재** |
| phonon | 2×2×2, 변위 **0.01 Å** + **DFPT** (⚠ 동시 서술 = 모순) | ⛔ **+U 를 켠 채인지 미기재** |

#### (b) ⛔⛔ **4f 를 "열어 놓고" 돌렸는데, 상태 선택 규칙이 없다**

Er³⁺ = 4f¹¹, Nd³⁺ = 4f³ — **둘 다 열린 껍질**이다. 그 위에 **+U 를 걸었다.**
이 조합은 **다중 admissible state** 를 만드는 교과서적 상황이다:

1. **4f 점유행렬의 준안정 극소** — +U 계산이 다른 f-궤도 점유에 갇히는 것은 희토류/악티나이드 계산의 가장 잘 알려진 병리다. 같은 구조에서 **수백 meV ~ eV 급의 서로 다른 SCF 해**가 나온다.
2. **스핀 배열** — 셀 안에 RE 가 1–5개다. FM / AFM / 무작위 정렬이 전부 다른 에너지를 준다.
3. **스핀분극을 켰는지조차 안 적혀 있다.** ISPIN=1 이면 4f 는 애초에 잘못 기술되고, ISPIN=2 면 초기 자화·NUPDOWN 규약이 필요하다.

**우리 판정 기준**(CLAUDE.md §계산 규율, 회신 N 문구):
> *admissible state 가 여럿인데 **선택·집계 규칙이 없으면 스칼라 보고량은 정의되지 않는다.** 열린 껍질 · 자성 · 산화환원 활성은 그 위험 신호다.*

**이 논문은 그 신호 세 개 중 두 개(열린 껍질 · 자성)를 켠 채, 선택 규칙을 한 줄도 적지 않고, 스칼라 총에너지로 조성을 골랐다.**

#### (c) ★ 신규 — **그 대가가 Table S1/S2 의 비단조성으로 이미 보인다** (가설, 확정 아님)

조성 선택의 근거인 "cohesive energy" 는 x 에 대해 **비단조**다(§5.1). 계단 차이를 보면:

```
Er 계 (Table S1)  x: 0     0.125  0.25   0.375  0.5     0.625
                  E: −19.24 −19.70 −20.15 −20.56 −22.02  −21.51
             Δ(계단):   −0.46  −0.45  −0.41  −1.46!  +0.51        ← 선택된 x=0.5 가 정확히 그 튀는 계단
Nd 계 (Table S2)  E: −19.24 −19.67 −20.71 −21.45 −20.75  −21.14
             Δ(계단):   −0.43  −1.04  −0.74  +0.70   −0.39        ← 선택된 x=0.375 직후에 부호 반전
```

> ⚠ **가설(확정 아님)**: 0.4–1.5 eV 규모의 이 도약은 **화학이 아니라 서로 다른 4f 점유행렬/스핀 basin 에 떨어진 결과**일 수 있다.
> 반증도 가능하다 — 배열(어느 Zr 를 바꾸고 보상 Li 를 어디 넣었는지)이 x 마다 달라 생긴 **배열 에너지**일 수도 있다.
> **그러나 두 후보 원인 모두 논문이 통제하지 않은 자유도이고, 저자들은 이 값을 그대로 "열역학적 안정성"으로 읽어 합성 조성을 정했다.**
> ⛔ 그리고 **참조상태(E_Li, E_Zr, E_M, E_Cl)가 미기재라 우리가 재계산해 가릴 수도 없다**(§5.1). **E_hull·형성에너지·분해검사 0건.**

#### (d) ⇒ 값 대조가 성립하는가

> ⛔ **성립하지 않는다.** 우리는 **frozen-4f**(4f 를 core 에 얼린 PAW)로 돌리고, 저들은 **open-4f + U(값 미공개)** 로 돌렸다.
> **두 계산은 Nd 의 전자구조를 다른 물건으로 취급한다.** 총에너지·결합길이·전하·상태밀도 어느 것도 나란히 놓을 수 없다.
> 유일하게 옮길 수 있는 것은 **절차적 교훈**(§21.9)이다.

---

### 21.6 ★6 **AIMD · 전도도 규약 전수**

| 항목 | 값 | 우리 규약 대비 |
|---|---|---|
| 방법 | **AIMD (VASP)** — MLIP 아님 | 우리는 UMA-s-1p1 MLIP-MD |
| 셀 | **2×2×2 = 72–77 원자**, **Li 16–21개** | ⛔ **MSD 를 내는 Li 가 20개 남짓**. 우리 MLIP 셀보다 한참 작다 |
| 앙상블 | **NVT, Nosé–Hoover** | 우리는 Langevin NVT (friction 0.02) |
| 온도 | **600 / 700 / 800 / 900 K (4점)** → **300 K 외삽** | 우리는 600/800/1000 K 3점(400/500 K 제외 판정) |
| Δt | **2 fs** | 동일 |
| 길이 | **총 50 ps** | 우리 규약은 **equil 5 ps + prod 200 ps** — **4배 길다** |
| **평형화 분리** | ⛔ **언급 없음** | 우리는 명시 |
| **MSD 창** | ⛔ **미기재** (Fig 3a–c 는 0–40 ps 전 구간 표시) | 우리는 **2–50 ps 고정** (`convention_check.py` 가 감시) |
| **절편** | ⛔ 미기재 (SI eq 3 은 `D = MSD/(2dΔt)` = 원점 강제형) | 우리는 **자유절편** |
| **차원인자 d** | **3** (SI: *"migration path is mostly anisotropic, so d is 3"* ← 문장이 비논리) | |
| **Haven 비** | **보정 없음 (H_R = 1)**, NE `σ = nq²D/k_BT` | 우리도 Haven=1 — **같은 규약** |
| **시드 수** | **1** | 우리는 **멀티시드 판정 필수**(단일시드 1.33× 철회 선례) |
| **배열 수** | **조성당 1개** | |
| **오차막대** | ⛔ **표·그림 어디에도 없다** | 우리는 600 K 3-시드 |
| k-mesh | **Γ only** | |
| 후처리 | **pymatgen** | |

#### ★ 신규 — **matched-x 로 다시 보면 원소 순위가 뒤집힌다** (Table S5 재배열)

같은 x 에서 Er 과 Nd 를 직접 붙이면(1차 패스가 세로로만 봤던 표를 가로로 본 것):

| x | σ_Er (mS/cm) | σ_Nd (mS/cm) | Nd/Er |
|---:|---:|---:|---:|
| 0.125 | 0.115 | 2.409 | **×21.0 (Nd 압승)** |
| 0.25 | 0.261 | 2.230 | **×8.5 (Nd)** |
| 0.375 | 0.729 | 3.560 | **×4.9 (Nd)** |
| 0.5 | **4.004** | 0.248 | **×0.06 (Er 압승, 16배)** |
| 0.625 | 2.435 | 0.243 | **×0.10 (Er, 10배)** |

> ⛔⛔ **한 계열 안에서 σ 가 Er 34.8배 · Nd 14.7배 흔들리고, 두 원소의 우열이 x=0.375 와 0.5 사이에서 통째로 뒤집힌다.**
> **50 ps · Li 16–21개 · 단일시드 · 조성당 배열 1개**로 낸 값에서 이런 진동은 **화학이 아니라 배열/시드 잡음으로 읽는 것이 자연스럽다.**
> 이것이 §21.1c(보상 Li 자리를 문서화하지 않았다)의 **정량적 대가**다.
> 그리고 §20.3 에서 확정된 것 — 헤드라인 Eₐ 가 **Methods 에 없는 500 K 점 하나**로 만들어졌다는 것 — 과 같은 뿌리다: **통계가 결론을 지탱하지 못한다.**

**실험 쪽**: 단일 온도(RT) EIS 뿐이라 **실험 Eₐ 가 없다.** 펠릿 φ16 mm × 1–2 mm, 1 MHz–0.1 Hz, `σ = L/(R·S)`.
등가회로·bulk/GB 분리·펠릿 밀도·압력·블로킹 전극 **전부 미기재**이고, **명시 기하로는 인쇄 σ 가 재현되지 않는다**(§20.5, 2.56배).

---

### 21.7 ★7 **할라이드 → 황화물 argyrodite 로 옮길 때 깨지는 가정**

| # | 그들 계에서 성립하는 가정 | 우리 계에서 왜 깨지나 |
|---|---|---|
| **1** | **RE 가 들어갈 8면체 양이온 자리가 이미 있다** ([ZrCl₆] → [NdCl₆], CN 6 → CN 6) | 우리 Rietveld 모델은 Nd 를 **P 자리(4a, PS₄ 사면체 중심)** 에 넣는다. **CN 4 자리에 CN 6 이온**이다. 반경 차가 **0.17 → 0.983 Å = 5.8배**. 실제로 우리 어닐본에서 Nd 는 사면체를 유지하지 못하고 **S 4–6개 + Cl 케이지**로 풀려 나간다(§21.4c) ⇒ **"P 자리 치환"이라는 이름이 이완 후 실체를 안 가리킨다** (cf. `kb/methodology/name_vs_substance_2026_09_09.md`) |
| **2** | **Φ 서술자의 보정 범위 안에 있다** (18계, 0.0200–0.1017 pm⁻¹, 전부 8면체 MCl₆) | 우리가 바꾸는 자리는 **P⁵⁺(IV) Φ = 0.294 pm⁻¹** — **그들 표 최댓값(Mo 0.1017)의 2.9배**, 범위 **밖**이다. 게다가 배위수도 음이온도 다르다. **Φ 를 우리 PS₄ 에 적용하는 것은 3배 외삽 + 배위 변경 + 음이온 변경의 삼중 외삽이다** |
| **3** | **전하보상이 +1 Li/도펀트** (Zr⁴⁺→M³⁺) | 우리는 **P⁵⁺→Nd³⁺ = +2 Li/도펀트**. 보상 부담이 **2배**이고, 그만큼 배열 자유도도 크다 |
| **4** | **음이온이 1가 Cl⁻ 하나** | 우리는 **S²⁻ · Cl⁻ · O²⁻ 셋**이고 그중 둘이 2가다. "M–X 정전기 세기"라는 단일 축이 성립하지 않는다. 게다가 **S/Cl 자리무질서 자체가 우리 계의 주 변수**(우리 modelc 규율) — 그들 계에는 없는 자유도다 |
| **5** | **음이온 골격이 곧 이동 골격** (Cl 8면체 network) | 우리는 **PS₄ 사면체라는 분자형 단위**가 있고, 그 회전/패들휠 자유도가 별도 축이다(`fang2022`). 골격 "연화" 의 의미가 다르다 |
| **6** | **σ 가 캐리어 부족으로 제한된다** (pristine Li₂ZrCl₆ = Li 2/f.u.) | 우리 argyrodite 는 **Li 5.4/f.u.** 로 캐리어가 이미 많다. **"aliovalent 로 Li 를 더 넣어 캐리어를 늘린다"는 이 논문의 주 기구가 우리 계에서는 훨씬 약하다** — 실제로 우리 Nd 결과는 σ 0.52×, Eₐ 사실상 불변이다(우리 db) |
| **7** | **산화안정성이 할라이드의 장점** (Cl⁻ 은 S²⁻ 보다 산화되기 어렵다) | 우리 axis ① onset 은 **S²⁻-limited** (comp1 = modelc = 2.256 V 동일). **할라이드의 고전압 안정 서사를 우리 계로 옮기면 축이 바뀐다** |
| **8** | **질량 효과가 작다** (Zr 91 → Nd 144, ω ×0.795) | **P 31 → Nd 144, ω ×0.463.** 우리가 VDOS 를 잰다면 **질량 항이 그들보다 훨씬 크고, 먼저 빼내지 않으면 "연화" 를 완전히 오독한다**(§21.3c) |
| **9** | **셀에 O 가 없다** | 우리는 **O 공도핑**이 핵심이고, O 가 Nd 와 **2.19–2.38 Å** 로 짧게 붙는다. 이 논문에는 **대응물이 아예 없다** — Nd–O 화학에 대해 이 논문은 아무 말도 해 주지 않는다 |

---

### 21.8 ★8 **프리프린트인가 게재본인가 — 프리프린트다** (재확인)

이번 업로드본에서 **직접 다시 확인**했다:

- 매 쪽 하단: *"Posted on 7 July 2026 — … **This is a preprint and has not been peer reviewed. Data may be preliminary.** — https://doi.org/10.22541/authorea.15005774/v1"*
- 표지 subject: `Authorea 2026.2026` · PDF title 메타 = 논문 제목 · producer `iText 4.2.0`
- 원고가 **Wiley 템플릿**(*"Article category: Full Paper / Subcategory: Lithium Ion Batteries"*)이고 **Received / Revised / Published online 이 전부 공란** = 투고 원고 상태.
- **버전 v1** 그대로. 2026-07-28 1차 패스 때와 **같은 v1** 이고, 게재본 흔적(권/호/페이지·DOI 변경)은 없다.

> ⛔⛔ **결론: 동료심사 전 프리프린트(Authorea v1, posted 2026-07-07).**
> **인용 문장마다 "Ren et al., Authorea 프리프린트(동료심사 전), 2026" 병기 필수** — 문서 머리 §⛔⛔ 규율 1 그대로.
> ⚠ 게재 여부는 **DOI 조회로 확정한 것이 아니라 PDF 표지·워터마크로만 판정**했다(이 환경에서 외부 조회를 하지 않았다). 출판본이 나왔는지는 별도 확인이 필요하다.

---

### 21.9 ★★★ **§우리 Nd 캠페인에 주는 것 — 판정 초안**

> **판정 대상**: *"우리 Nd 전하보상 · 4f 처리를 이대로 두어도 되는가?"*
> **근거**: ★1(§21.1) · ★4(§21.4) · ★5(§21.5) + 우리 `ndo_lpscl16` 보고량 카드 · 구조 6점 실측.
> ⚠ 이것은 **판정 초안**이다. `db/governance/decisions.json` 등록은 1저자 ratify 사항이고, 이 digest 는 등록하지 않는다.

#### ① 전하보상 — **✅ 이대로 둔다.** (외부 선례가 우리와 같은 방식이다)

| | ren2026 (할라이드) | **우리 `ndo_lpscl16`** |
|---|---|---|
| 치환 | Zr⁴⁺ → Er³⁺/Nd³⁺ (8면체) | **P⁵⁺ → Nd³⁺** (Rietveld: `Nd8` 이 `s2 P` 와 같은 (½,½,½), occ 0.02) |
| 부족 전하 | −1 / 도펀트 | **−2 / 도펀트** |
| 보상 방식 | **화학식에 Li 추가, 중성 셀** | **동일 — Li 추가, 중성 셀** |
| 검산 | N_Li = 8(2+x) 로 σ 11/11 재현(§6.3) | `Cl₆Li₂₆Nd₂O₃P₂S₁₅` → 양 (+26 +6 +10)=+42 / 음 (−6 −6 −30)=−42 **✅ 정확히 0**<br>`Cl₈Li₃₁Nd₂O₃P₃S₁₉` → +52 / −52 **✅ 0** (모(母) 대비 P 2개↓, Li 4개↑ = **+2 Li/Nd**) |
| 배경전하 | 없음 | **없음** |
| 공공 스킴 | 없음 | 없음 |

> ✅ **판정: 우리 보상 스킴(중성 셀 + 명시적 Li 추가, 배경전하·전하보정 없음)은 이 분야에서 통용되는 방식이고, 이 논문이 그 외부 선례다.**
> ⛔ **단 선례가 보증하는 것은 "회계"까지다.** 이 논문은 **보상 Li 를 어느 자리에 넣었는지 안 적었고**(§21.1c), 그 대가로 **matched-x 순위가 뒤집혔다**(§21.6).
> 🔧 **그래서 우리에게 붙는 조건 2개**:
> - **(A) 보상 Li 자리를 카드에 명시적으로 적는다.** 우리 6구조는 조성이 같고(`SUMMARY.json` 확인) 회계도 맞지만, **"추가 Li 4개를 어디에 놓았는가"가 카드 §2·§3 어디에도 없다.** 지금 §3 위험요인은 **O 배열 3종만** 열거한다 — **Li 배치는 열거되지 않은 자유도**다. 최소한 *"Li 배치는 Rietveld 부분점유에서 UMA 어닐로 결정했고 열거하지 않았다"* 를 **한계로 적어야** 한다.
> - **(B) 배열 잡음을 G3 로 이미 막고 있다 — 유지한다.** 카드 G3(1·2위 차 < 50 meV 면 "구별 불가")·G2(두 셀이 다른 배열을 가리키면 **선택 실패로 보고**)는 정확히 이 논문이 안 가진 장치다. **이 논문은 우리 G2·G3 의 존재 이유를 보여주는 반례 사례로 쓸 수 있다.**

#### ② 4f 처리 — **✅ frozen-4f 를 유지한다. 이 논문은 오히려 그 결정을 지지한다.**

우리: `Nd.pbe-spdn-kjpaw_psl.1.0.0.UPF`, `z_valence = 11` → **4f 를 core 에 얼렸다.**
카드가 이미 정직하게 적어 뒀다: *"4f 다중항 문제를 **회피한 것**이지 푼 것이 아니다"* + **"⛔ Nd 4f 에 관한 어떤 주장이든 금지"**.

> ✅ **판정: 지금 우리가 하는 일 — 같은 셀 안 O 배열의 총에너지 *차이*로 배열 하나 고르기 — 에 대해 frozen-4f 는 옳은 선택이다.**
> 이유 세 가지:
> 1. **우리 보고량이 ΔE(같은 셀 안의 차)** 라서, Nd 4f 가 **모든 배열에서 동일하게 얼어 있으면 그 항이 정확히 상쇄된다.** open-4f 였다면 배열마다 **다른 4f basin** 에 떨어져 ΔE 에 **수백 meV 의 가짜 신호**가 섞일 수 있다 — 그러면 우리 G3 컷(50 meV)은 무의미해진다.
> 2. **이 논문이 그 실패의 실물이다**(§21.5c): open-4f + U(값 미공개) + 상태선택 규칙 없음 → **E_coh 계단이 0.4–1.5 eV 로 튀고, 그 튀는 점이 곧 그들이 고른 조성이다.**
> 3. 우리 규율 문구 그대로 — **"admissible state 가 여럿인데 선택·집계 규칙이 없으면 스칼라 보고량은 정의되지 않는다."** frozen-4f 는 4f 상태 공간을 **1개로 축소**해 이 조건을 **정의상 만족**시킨다.
>
> ⛔ **대신 값을 치른다. 그 값을 카드에 그대로 유지한다:**
> - **Nd 4f 관련 주장 전면 금지** (이미 카드 §5 금지목록에 있다 — 유지).
> - **Nd 의 자성·f-전자 상태밀도·f-p 혼성·Nd 산화수 변화 서술 금지.** 하류 ICOHP/PDOS 에도 이 꼬리표가 그대로 간다.
> - **이 논문의 4f 관련 수치와 우리 값을 같은 표에 놓지 않는다** (§21.5d).
>
> 🔧 **보완 1건(카드가 스스로 지목한 약한 자리)**: 카드 §3 의 *"자성 basin 이 여럿인가 → **미판정**, nspin 을 못 박지 않았다"*.
> **frozen-4f 라도 이건 닫아야 한다** — `~/work/runs/nd_scf_2026_09_08/*/scf.in` 의 `nspin`/`starting_magnetization` 을 읽어 카드에 적고,
> **6점이 같은 값인지** 확인한다. 6점 중 하나만 다르면 그게 SDCP phaseB 의 *"제약된 기준에서 자유로운 대상을 뺐다"* 와 같은 부류가 된다.

#### ③ 그러면 이 논문에서 **가져올 수 있는 것**은

1. **선례로서의 전하보상 스킴** — "중성 셀 + 명시 Li" 는 문헌 표준이다(§21.1b). ⛔ 수치는 안 가져온다.
2. **반례 카탈로그** — 서술자(Φ), 단일배열 AIMD, 정규화 없는 VDOS 비교, open-4f + U 무규약. **우리 카드의 G2·G3·보고량 카드가 왜 필요한지에 대한 외부 실증**이다.
3. **⟨ω⟩ = ∫ωg(ω)dω / ∫g(ω)dω** 라는 값싼 스칼라(§7.4) — 단 **질량 항을 먼저 빼야 한다**(§21.3c, 우리 계는 −53.7 %).
4. **축분해 MSD** 제시 양식(§9.2) — 우리 MSD 는 등방 총합만 낸다.

#### ④ **가져오면 안 되는 것** (이 절의 금지목록)

- ⛔ σ · Eₐ · D · NEB 장벽 · 격자상수 · E_coh **절대값 전부** (기존 규율 + §20.11).
- ⛔ **"Nd 도핑이 σ 를 5–6배 올린다"** 를 우리 Nd 옆에 놓기 — **우리 Nd 는 σ 0.52×, Eₐ 사실상 불변**이다. 계·자리·전하보상·4f 처리가 전부 다르다. **병치 자체가 오도다.**
- ⛔ **"저-Φ 가 좋다"** 명제 (§21.2c: 그들 자신의 Er/Nd 대조가 네 축 모두에서 반대).
- ⛔ **"Nd 가 격자를 무르게 한다"** (§21.3: 연화가 정량화되지 않았고 질량 항이 안 빠졌다. §21.4: 우리 Nd–O 는 오히려 짧다).
- ⛔ **Nd–Cl / Nd–O 결합길이** — **이 논문에 없다.** 없는 값을 "문헌값"으로 만들지 않는다.

#### ⑤ 다음에 할 일 (우선순위)

| 순위 | 무엇 | 왜 지금인가 |
|---|---|---|
| **1** | 카드 §6 `계산_설정` 채우기 — **`nspin`/`starting_magnetization` 6점 동일성 확인** | 카드가 스스로 "가장 약한 자리"로 지목. §21.5 가 그 위험의 실물을 보여줬다 |
| **2** | 카드에 **"보상 Li 배치는 열거하지 않은 자유도"** 를 한계로 명시 | §21.1c ↔ §21.6 (문서화 안 한 자유도가 순위를 뒤집는다) |
| **3** | 어닐본 **Nd 배위(Nd–O/S/Cl)를 정식 측정으로 승격** | §21.4c 는 이 digest 에서 즉석으로 잰 값이다. G2 판정 시 *"O 환경(배위·이웃)으로 재확인"* 하기로 카드가 이미 정해 뒀다 |
| **4** | (중기) `tools/ionic/` UMA-MD 궤적에 **VACF → VDOS → ⟨ω⟩** 후처리 | §7.4 채택 권고 + §21.3c **질량 정규화를 처음부터 넣을 것** |

---

### 21.10 3차 패스 종합 — 신규 8건

| # | 항목 | 종전 | 3차 패스 |
|---|---|---|---|
| 1 | **도펀트 자리** | 화학식에서 추정 | ✅ **확정 — Zr 8면체. `Fig. 2a` 다면체 갤러리 4개가 전부 8면체** |
| 2 | **전하보상 회계** | §6.3 σ 역산으로 N_Li 확정 | ✅ **의미 확정 — 도펀트당 Li 원자 1개 추가, 중성 셀, 배경전하 없음** |
| 3 | **보상 Li 자리** | — | ⛔ **신규 — 논문 어디에도 없다.** 배열 열거·SQS·선택규칙 0 |
| 4 | **서술자 내부 반례** | 문헌 15편 가로지른 반례 3건 | ⛔⛔ **신규 — Er vs Nd 내부 대조에서 NEB·NEB·σ_AIMD·σ_exp *네 축 모두* 서술자와 반대** |
| 5 | **VDOS 정규화** | y 스케일 다름(판독) | ⛔⛔ **신규 — 모드 수 차 5.6 % 인데 진폭 4.6배 + 지지구간은 도핑계가 더 넓다 = 패널별 임의 정규화. 자기 최대로 정규화하면 "1–2 THz 증가" 가 44–46 % 로 사라진다** |
| 6 | **Cl 밴드 상단** | — | ⛔ **신규 — 세 패널 모두 ≈5.0–5.1 (arb.) 로 안 움직인다.** 늘어난 것은 Li 부분스펙트럼(Li 를 더 넣었으니 당연) |
| 7 | **질량 효과** | — | ⛔⛔ **신규 — Zr→Er/Nd 로 M-투영 모드가 k 불변이어도 20–26 % 내려간다. 논문은 이 항을 언급조차 안 한다** |
| 8 | **matched-x 순위** | 세로(농도별)로만 봄 | ⛔⛔ **신규 — x=0.375 와 0.5 사이에서 Er↔Nd 우열이 통째로 뒤집힌다(×4.9 Nd → ×16 Er). 단일시드·단일배열 50 ps 의 한계** |

> ### ⛔ 인용 규율에 **추가**되는 항목 (§20.11 박스에 이어서)
> 7. **"lattice softening"·"저주파 모드 증가" 를 이 논문 근거로 인용 금지.** 세 VDOS 패널은 정규화가 달라 비교 불가이고(§21.3b), 질량 항이 안 빠졌다(§21.3c).
> 8. **"Nd–Cl 결합이 길어진다/약해진다" 를 수치와 함께 인용 금지 — 이 논문에 결합길이가 없다**(§21.4a). 이온반경 합(2.79 Å)은 **우리 산수**이지 이 논문 값이 아니다.
> 9. **이 논문의 Nd 결과를 우리 `ndo_lpscl16` Nd 옆에 병치 금지.** 계·자리(8면체 Zr vs 사면체 P)·보상(+1 vs +2 Li)·4f(open+U vs frozen)가 전부 다르다(§21.7).
> 10. **"DFT+U 로 4f 를 제대로 다뤘다" 로 이 논문을 인용 금지** — U 값·스핀·점유행렬 정책이 전부 없어 **재현 불가**이고, 조성 선택 근거인 E_coh 가 그 위에 서 있다(§21.5).
