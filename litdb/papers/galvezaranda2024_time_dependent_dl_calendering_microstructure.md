...
2. **max-pooling 3 개가 전부 항등 연산**이다 (pool size = stride = 1).  논문은 *"계산 복잡도를 줄인다"*
   고 쓰지만 **아무 것도 안 한다**.
3. **커널 3 짜리 conv 의 출력 길이가 2** ⇒ `padding='valid'` 면 **입력 시퀀스 길이가 4**,
   `padding='same'` 이면 **2** 여야 한다.  **본문이 말하는 3-lag 와 어느 쪽도 안 맞는다.**
   (Fig 1b 캡션은 *"CD 가 모델의 입력으로 쓰인다"* 고 하므로 **네 번째 슬롯 = CD 조건화**일 가능성이
   있으나, **논문은 CD 를 어떻게 넣는지 끝까지 설명하지 않는다.**)
...
| **2** | 1D-CNN 아키텍처 도해(input → conv/BN/pool/dropout/FC → output) | — (Table S1 이 정보량이 더 많다) |
| **3** | ★ 전체 워크플로: DEM → 복셀화 → 시계열 재배열 → CNN 학습 → **2 종 지표(데이터 / 기능)** 평가 | ★ **"두 층 검증"(픽셀 지표 + 물리 기능지표) 도식** — 우리 ML 예측기(§8-③) 문서에 그대로 쓸 틀 |
...
**① 그들 쪽 선결조건 — 추론 프로토콜이 미공개다 (teacher forcing 의혹).**
Fig 5c 의 R² 궤적(0·1·2 = 정확히 100 → 92.2 로 하강 → **프레임 9 에 정확히 100 복귀** → 91.7 최저 →
21 부터 96.8 평탄)은 **free-running rollout 으로는 만들어질 수 없는 모양**이다 (자기회귀는 오차가
누적되어 되돌아오지 않는다).  Fig 6b·6d 도 같은 **하강 후 99 대 복귀** 패턴을 보인다.
⇒ **평가가 teacher-forced 라면, 프레임 m 을 내려면 DEM 프레임 m−3, m−2, m−1 이 있어야 하고
그러면 DEM 을 어차피 돌려야 하므로 188× 는 surrogate 가속이 아니다.**
⚠ **논문은 어느 쪽인지 끝까지 말하지 않는다.**  Fig 1b 는 rollout 처럼 그려져 있다.
**이 한 가지가 이 논문 전체의 가치를 가르는데 미공개다.**  ⇒ 우리가 따라 하려면
**rollout / teacher-forcing 을 반드시 명시하고 둘 다 보고해야 한다.**
...
### ⑤ ★★ **추론 프로토콜 미공개 — teacher forcing 의혹** (§7-2-① 상세)
증거: Fig 5c 에서 **프레임 0/1/2 = 정확히 100** (입력 lag) → 3/4/5 = 95.6/93.7/**92.2** →
**프레임 9 = 정확히 100 으로 복귀** → 16–18 = **91.7 최저** → 21 이후 **96.7–96.9 평탄**.
Fig 6b 도 87.2 로 떨어졌다가 **99.5–99.8 로 복귀**한다.
**자기회귀 rollout 은 오차가 누적되므로 "정확히 100 으로 복귀" 를 만들 수 없다.**
⇒ **매 프레임 실제 DEM lag 를 먹이는 one-step-ahead 평가**로 보이고, 그렇다면
**"DEM 을 안 돌려도 된다" 는 함의가 성립하지 않는다.**
⚠ **논문에 어느 쪽인지 적혀 있지 않다.**  Fig 1b 모식도만 rollout 처럼 보인다.
⇒ **이 논문을 인용할 때는 반드시 이 불확실성을 함께 적어야 한다.**
