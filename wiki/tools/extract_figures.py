...
try:
    ...
    sys.exit(
        ...
        "     python3 -m venv .venv   (저장소 root 에서 — bash scripts/setup-wsl.sh 가 해 준다)\n"
        "     .venv/bin/pip install -q pymupdf pillow   (setup-wsl.sh 가 이미 넣는다)\n"
        "   그 다음부터는 python3 대신 이걸로 실행:\n"
        "     .venv/bin/python wiki/tools/extract_figures.py --inbox\n"
        "   (자주 쓸 거면 ~/.bashrc 에)  alias litfig='.venv/bin/python "
