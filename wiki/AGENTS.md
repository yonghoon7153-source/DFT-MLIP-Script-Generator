# AGENTS.md — LLM Wiki Harness (portable, Li2S ASSB mothership)
...
Operation 의 정식 절차는 repo root `.claude/commands/{operation}.md` 에 마크다운으로 있다. `/` 트리거는 Claude Code 전용이지만 파일은 어느 에이전트나 읽을 수 있다 — operation 전에 해당 파일을 읽고 따른다. 논문 에이전트의 절차는 `.claude/agents/paper-curator.md`.

/wiki-ingest · /wiki-inbox · /wiki-query · /wiki-verify · /wiki-lint · /wiki-status · /wiki-wrap · /paper · /seminar · /compare
...
공유 remote(origin)가 있으면 **세션 시작 시 `git pull --rebase`**, 마무리는 `/wiki-wrap` 절차가 commit+push 한다. `log.md` 충돌 시 양쪽 항목 모두 보존한다 (append-only).
