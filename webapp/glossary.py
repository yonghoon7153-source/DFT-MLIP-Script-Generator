...
CATS_G = ["기초 이론", "전자 구조", "화학 결합", "기계 물성", "이온 수송", "계면·안정성",
          "MLIP·자동화", "방법론·모델링", "연구 규율"]

# ⚠ 용어 규율 (2026-09-01, 1저자 결정) — **사람이 읽는 표면**인 이 파일은
#   `estimand` → 보고량 · `canary` → 대조 잡 · `claim ceiling` → 허용 서술 범위 로 쓴다.
#   영문은 괄호로만 병기한다. ⛔ 기계 필드명(`kind: "estimand"` · `canary_geometry`)과
#   파일명(`estimand_card.md`)은 **안 바꾼다** — 개서하면 원장·게이트가 끊긴다.
...
{"id":"arrhenius", "doc":"md", "doc_sec":"§5 Arrhenius — 활성화에너지","term":"Arrhenius / Ea","full":"활성화에너지 · 전도도","cat":"이온 수송",
 ...
 # 회신 BG ② — 철회값·보류주장은 **data-claim 으로 이름을 대야** 결속이다 (근접 표지로는 부족).
 "ours":"<b>단일 궤적 deck 앵커</b>: comp1 0.253 / modelc 0.224 (comp1↔modelc 비교 전용, 시드 오차막대 없음). <b>멀티시드</b>: modelc 0.197±0.032 (3-seed×3-T) · <span data-claim='MD_Ea_eV@b2o3'><s>b2o3 0.199±0.034</s> <b style='color:#be123c'>⛔ 철회 (2026-08-23)</b> — 전구간 단일 직선 적합인데 아레니우스가 800 K 위에서 굽는다(600→800 0.222 / 800→1000 0.077 eV). 저온 구간 Ea 로만 쓴다</span> / LPSOCl 0.287±0.024 (4-seed×3-T) / comp2 0.275±0.033 (3-seed). <span data-claim='HZ-cross-system-Ea'>LPSOCl vs modelc = +90 meV(둘 다 멀티시드) <b style='color:#c05621'>⛔ 계간 비교 보류 (회신 AK)</b></span>. comp2는 800K 시드 산포가 커서 300K 외삽 σ 비율 판정 보류(0.12–1.48×, inconclusive). ⚠ 프로토콜이 다른 값끼리 빼면 안 됨."},
...
{"id":"fixed_cell_eos","term":"고정셀 + EOS (vc-relax 안 함)","full":"fixed-cell relaxation + Birch\u2013Murnaghan EOS \u2014 왜 셀을 안 푸는가","cat":"방법론·모델링",
 "what":"부피는 <b>등방 스케일 + E(V) BM3 적합</b>으로 잡고, 그 V\u2080 에서 <b>이온만</b> 완화한다. 셀 형상은 건드리지 않는다(vc-relax 금지).<br>⚠ 흔한 설명은 \"argyrodite 가 cubic 이라 보존해야 해서\" 인데 그건 <b>이유의 절반</b>이다. 더 정확히는 — <b>우리 셀은 애초에 cubic 이 아니다.</b> 진짜 Li\u2086PS\u2085Cl 이 cubic 인 것은 Li 가 48h/24g 를 <b>동적으로</b> 돌아 <b>시간평균</b>이 cubic 이기 때문이고, 0 K 정적 계산에 Li 배치 하나를 얼려 넣으면 그 구성이 cubic 일 이유가 없다. vc-relax 를 걸면 <b>그 스냅숏의 최소점</b>으로 가서 셀 뒤틀림이 <i>\"어떤 배치를 골랐나\"</i> 의 인공물이 된다.<br>\u21d2 셀 고정은 <b>가진 대칭의 보존이 아니라, 정적 계산이 알 수 없는 평균 대칭을 사람이 넣어 주는 것</b>이다.<br>\u26a0 <b>적용 범위</b>: 이 정책은 <b>DFT 경로에만</b> 걸려 있다 — 같은 README 다음 줄이 \"Stage 2a: <b>UMA full relax</b>\" 이고, UMA 기본 경로(<code>run_uma_screening.py:56</code>)는 <b>형상 무제한 CellFilter</b> 다 (<code>cell_policy_gap_2026_09_13.json</code> GAP-1·2).",
 "how":"① 96\u2013108 % 등방 부피 격자 \u2192 각 점에서 <code>calculation=\'relax\'</code>(셀 고정, 이온만) \u2192 ② E(V) 를 BM3 로 적합해 V\u2080\u00b7B\u2080 \u2192 ③ V\u2080 에서 다시 이온만 완화 = 생산 구조.<br><b>왜 EOS 가 정공법인가</b>: 등방 스케일 + E(V) 적합은 <i>cubic 을 강제한 채 셀을 최적화하는 것</i>과 같은 일인데, 유한 기저에서 응력을 직접 최소화할 때 생기는 <b>Pulay stress</b> 를 피해 간다. ecutwfc 60 USPP 에서 vc-relax 로 부피를 찾으면 그 함정에 그대로 걸린다.",
 "ours":"<b>규약 출처 4곳</b>: <code>cascade_v23_review_2026_07_11.md:28</code>(\"파이프라인 v2 원칙: vc-relax 없음\") \u00b7 <code>modelc_nd_doped.json:80</code>(\"vc-relax NOT used (distorts argyrodite)\") \u00b7 <code>b2o3_champion_coordination_2026_06_29.md:13</code> \u00b7 <code>lpsocl_candidates/README.md:24</code>(\"NO vc-relax, DFT = fixed-cell only\").<br><b>얼린 구성이 cubic 이 아니라는 직접 증거</b>: <code>elastic.json</code> 의 <code>_Zener_A_convention</code> \u2014 comp1 relaxed 의 대칭동등 삼중항이 <b>1.144 / 0.751 / 0.918</b> 로 갈린다(산포가 인용하려던 문헌 간극 0.17 보다 크다).<br><b>대가 = 잔류 편차응력</b> (comp1_V0_k444 실측, <code>static_pair_dft_2026_09_13.json</code> \u2014 ⚠ <b>status: proposed</b>, 1저자 비준 전): 평균압 <b>\u22120.002 GPa</b>(등방은 0)인데 편차 <b>\u00b11.3 GPa</b>, <b>yz 전단 1.08 GPa</b>. C\u2084\u2084 = 18.98 GPa 로 나누면 전단변형 <b>~5.7 %</b> 어치다. \u26d4 없앨 수 없다(없애려면 cubic 을 깨야 한다) \u2014 <b>고칠 결함이 아니라 명시할 사실</b>이다. \u26a0 <b>단 \"모든 구조가 같은 편차응력을 공유한다\" 고 쓰면 과하다</b>(회신 BP 정정) \u2014 형상 고정은 편차응력을 <b>허용</b>할 뿐 같은 크기나 비영 값을 <b>강제하지 않는다</b>. 위 값은 <b>comp1_V0_k444 의 값</b>이지 파이프라인의 상수가 아니다.<br><b>\"0 GPa\" 의 정확한 표현</b>(회신 BP): <i>\"셀 각도와 길이비를 고정하고 등방 E(V) 경로에서 부피를 최적화한다. <b>목표</b> 평균압은 0 GPa 이며, <b>실제 잔류 평균압과 편차응력을 별도로 보고한다</b>.\"</i> \u26d4 \"전체 응력 0\"\u00b7\"자유 영응력 평형\" 과 다르다.<br><b>\u2605 이게 무는 자리 = 탄성 Cij</b>: 응력을 진 기준상태의 stress\u2013strain Cij 는 표준 Cij 가 아니다(Wallace 류 보정항이 관행적으로 무시된다). 그리고 b2o3 전단 실패(<code>elastic.json</code>: \"\u00b1shear \u2192 DIFFERENT local minima \u2192 eigenvalue \u22122.87, E_VRH \u2212107.7 unphysical, <b>WITHHELD</b>\" \u2014 \u26d4 그 값들은 <b>인용 불가</b>이고 여기서는 <i>\"탄성이 깨졌다\"</i> 는 사실의 근거로만 쓴다. 조성 간 탄성 비교에는 <code>HZ-elastic-cross-composition</code>(CONDITIONAL)도 걸린다)와 <b>겹치는 메커니즘</b>이다 \u2014 기준이 이미 전단을 지고 있으면 \u00b1변형이 다른 골로 굴러가기 쉽다. \u21d2 <code>el</code> fit 때 <b>V0_relax.out 의 응력 블록</b>을 같이 회수한다(추가 계산 0). 자세한 것은 methods \u00a73-1."},
...
# ── 연구 규율 ─────────────────────────────────────────
# ⚠ 이 다섯 장은 이 repo 에서 **제일 자주 쓰는 말**인데 용어집에 정의가 0건이었다
#   (v3 조사 no-governance-vocabulary). 물리 용어가 아니라 **판정 용어**라서,
#   물리는 알고 이 repo 는 처음인 사람이 정확히 여기서 막힌다.
# ⚠ 숫자를 여기 박지 않는다 — 원장이 늘면 손으로 쓴 개수는 반드시 낡는다.
#   결정 id 처럼 **안 변하는 이름**만 대고 세는 것은 /governance 에 맡긴다.
{"id":"estimand","term":"보고량","full":"보고량 (estimand) — 계산이 끝나면 무엇을 보고할 것인가","cat":"연구 규율",
 "what":"계산을 던지기 <b>전에</b> 확정하는 한 문장 — \"이 계산이 끝나면 나는 <b>무슨 양</b>을 <b>어떤 단위·부호·기준</b>으로 보고하는가\". 값이 맞나(정확도)와 다른 질문이다. <b>맞는 양을 재고 있나</b>를 묻는다.<br>판정 기준(회신 N): <b>허용되는 상태(admissible state)가 여럿인데 선택·집계 규칙이 없으면 스칼라 보고량은 정의되지 않는다.</b> 열린 껍질 · 자성 기판 · 산화환원 활성이 그 위험 신호다.",
 "how":"① <code>kb/templates/estimand_card.md</code>(부르는 이름 = <b>보고량 카드</b>) 의 §1–3 을 채운다 — 원하는 것 / 재는 식 / <b>이 계에서 그게 잘 정의되는가</b>. ② 리뷰에 보내는 건 계산 번들이 아니라 <b>그 세 절</b>이다. ③ 위험 신호에 걸리면 셋 중 하나를 고른다 — 상태를 선언해 <code>X(상태)</code> 로 정의하거나, 집계 규칙(최저/앙상블/분포)을 미리 적거나, <b>질문을 바꾼다</b>. ④ 던지기 전 30초: <code>grep -rl \"&lt;양이름&gt;\" kb/</code> — 이미 판정이 있으면 그게 이긴다.",
 "ours":"채택 배경은 실패다 — SDCP-doped 흡착에너지를 <b>여덟 번</b> 계산했고 여덟 번 반려됐다. 받은 리뷰는 전부 \"제대로 돌렸나\"(무결성·해시·INCAR·pin·게이트)였고 <b>전부 통과</b>했다. \"맞는 양을 재고 있나\"는 여덟 번째에야 물었고 즉시 P0 가 나왔다. ⚠ \"일곱 번은 안 돌려도 됐다\" 는 <b>철회</b>(회신 N) — 카드를 블라인드로 재생했을 때 확실히 잡는 건 #7–8 정도다. 여덟 실패의 원인은 하나가 아니라 <b>층위</b>다.<br>지금 살아 있는 보고량 판정은 <a href=\"/governance\">판정 원장</a>의 <code>kind: estimand</code> 행이다(개수는 원장이 센다 — 여기 안 적는다). 카드 원본은 <a href=\"/files?q=estimand_card\">kb/templates/estimand_card.md</a>.<br>⛔ 이 카드가 <b>못 하는 것</b>: 계산이 맞는지는 안 본다. 질문이 성립하는지만 본다 — 통과했다고 결과가 옳은 게 아니라 옳을 <b>자격</b>이 생길 뿐이다."},

{"id":"closure_criteria","term":"닫힘 조건","full":"닫힘 조건 · 마감 카드 (closure criteria)","cat":"연구 규율",
 "what":"캠페인을 <b>끝났다</b>고 선언하기 위해 미리 박아 두는 조건, 그리고 채워졌을 때 남기는 문서. 네 칸이다 — <b>확정값</b> · <b>허용 서술(이대로만 쓴다)</b> · <b>⛔ 금지 서술</b> · <b>재개 조건(이것들만)</b>.<br>핵심은 <b>순서</b>다. 데이터를 보고 \"닫혔다\"고 판단하지 않는다 — <b>조건을 먼저 정하고, 그게 채워졌으므로 닫는다.</b> 순서가 뒤집히면 그건 마감이 아니라 지금 있는 데이터에 맞춘 사후 설명이다.",
 "how":"<code>db/properties/&lt;계&gt;_closed_&lt;날짜&gt;.json</code> 을 남긴다. 재개 조건은 <b>열거된 것만</b>이고 <b>그 밖의 이유로는 다시 열지 않는다</b> — \"새 자세를 하나 더 봤다\"는 재개 사유가 아니다. 비준(ratification)이 붙은 마감 카드는 본문을 한 글자도 안 고친다(지문이 깨진다). 고칠 일이 생기면 새 결정을 올려 <code>superseded</code> 로 잇는다.",
 "ours":"선례: <a href=\"/files?q=sdcp_neutral_closed\">sdcp_neutral_closed_2026_08_28.json</a> — 닫는 범위 · 확정값 · 허용 서술 · 금지 서술 7항 · 재개 조건이 한 파일에 다 있다. 금지 서술 칸이 실제로 일한다(예: \"술포네이트가 앵커링한다\"는 기전 근거 없음으로 금지, 실제 접촉은 C–H···표면).<br>⚠ 조건 없이 닫으면 물린다 — SDCP 는 조건 없이 <b>두 번</b> 닫았다가 두 번 되돌렸다(doped v1 · 회신 M 마감보류). 그래서 2026-08-28 에 \"닫힘 조건을 먼저 박는다\"가 규율이 됐다.<br>지금 살아 있는 마감 판정은 <a href=\"/governance\">판정 원장</a>의 <code>kind: closure</code> 행이다."},

{"id":"control_job","term":"대조 잡","full":"대조 잡 · 검증용 대조 계산 (control calculation)","cat":"연구 규율",
 "what":"본 계산과 <b>나란히</b> 돌려 \"내가 본 게 물리인가 도구인가\"를 가르는 계산. 게이트가 실제로 일하는지 확인하는 자리라, <b>결과를 보기 전에</b> 무엇을 대조로 쓸지 정해 둔다.",
 "how":"게이트는 통과/탈락만 묻는 게 아니라 <b>\"기준과 대상이 같은 제약인가\"</b>까지 물어야 한다. 자화·전하·스핀처럼 <b>상태를 고르는 축</b>이 있으면, 기준과 대상이 같은 <b>상태 선택 정책(state-selection policy)</b> 아래 있어야 뺄셈이 성립한다.",
 "ours":"실측 두 층 (2026-08-28 정정, <code>kb/methodology/estimand_before_running_2026_08_28.md</code> §2.1):<br>① phaseB 의 <code>mol_doped</code> 자화가 1 이 아니라 <b>0.175</b> 였다 — <code>ISMEAR=1</code> 설정 결함이다.<br>② wave1 은 자화가 <b>1.0000</b> 으로 예쁘게 나왔는데, <code>NUPDOWN=1</code> 로 <b>강제</b>한 값이라 검증이 아니다. 정작 복합체는 <code>NUPDOWN=−1</code> 자유로 돌아 −0.31 / 3.72 μB 에 떨어졌다 — <b>제약된 기준에서 자유로운 복합체를 뺐다.</b><br>⚠ 회신 O: 고치는 법은 \"전 계에 같은 NUPDOWN 값\"이 <b>아니라</b> 같은 <b>상태 선택 정책</b>이다 — 전부 자유 바닥상태로 가거나, 선언된 해리 채널과 양립하는 대응 제약을 양쪽에 건다.<br>⛔ 대조 잡이 <b>못 하는 것</b>: 통과해도 보고량이 잘 정의됐다는 뜻은 아니다. 그건 <b>보고량 카드</b>가 보는 층이다."},

{"id":"claim_ceiling","term":"허용 서술 범위","full":"허용 서술 범위 (claim ceiling) — 이 값으로 어디까지 말해도 되나","cat":"연구 규율",
 "what":"값이 맞아도 <b>그 값으로 할 수 있는 말</b>은 따로 정해진다. 허용 서술 범위는 그 천장이다 — 정확도가 아니라 <b>일반화의 한계</b>를 정한다. \"진공·0 K·단분자에서 잰 값\"은 그대로 \"실제 셀에서 더 잘 붙는다\"가 되지 않는다.",
 "how":"마감 카드의 <b>허용 서술 / ⛔ 금지 서술 쌍</b>이 이 천장이다. 허용 칸은 <b>문장 그대로</b> 적어 두고 그대로만 인용한다(요약·의역이 곧 천장 초과다). 금지 칸에는 실제로 우리가 하려다 막힌 문장을 적는다 — 추상적인 금지는 안 걸린다.<br>화면에서는 <code>data-claim</code> 결속이 이 천장의 기계 집행이다 — 철회·비인용 값을 그리는 자리는 <b>어느 주장인지 이름을 대야</b> 한다.",
 "ours":"작동 사례(<a href=\"/files?q=sdcp_neutral_closed\">sdcp_neutral_closed</a> 금지 서술): \"자리 불문\" · \"PTFE 보다 이긴다\" · PTFE <b>고분자</b>로의 확장 · 접착력·계면저항·실제 피복률로의 확장 · 타 코드·문헌 절대값과의 직접 비교. 값은 그대로인데 <b>말할 수 있는 범위</b>만 좁힌 것이다.<br>인용 위험 원장(<a href=\"/governance\">판정 원장</a>)이 같은 일을 축 단위로 한다 — 어떤 축은 값이 있어도 <b>인용 가능한 수가 0개</b>다.<br>⚠ 용어: 사람이 읽는 글에서는 \"허용 서술 범위\"라고 쓴다. <code>claim ceiling</code> 은 영문 병기용이다."},

{"id":"prereg_seal","term":"사전등록 · 봉인","full":"사전등록(preregistration) · 봉인(seal)","cat":"연구 규율",
 "what":"<b>사전등록</b>은 결과를 보기 전에 판정 규칙 — 판정량 · 문턱 · 게이트 · 실패 조건 — 을 파일로 박는 것이다. 결과를 본 뒤 문턱을 옮기면 그 문턱은 아무것도 못 걸러낸다.<br><b>봉인</b>은 그 시점의 입력·규칙·코드를 해시로 잠그는 것이다. 봉인의 뜻: <b>이 시각 이후의 실행은 이 봉인에 귀속된다 — 봉인 뒤에 파일이나 규칙이 바뀌면 그 실행은 이 봉인으로 설명되지 않는다.</b> 새로 봉인하고 다시 돌려야 한다.",
 "how":"<code>db/properties/*_prereg_*.json</code> 에 판정량과 문턱을, <code>*_seal_*.json</code> 에 <code>sources</code>(파일별 sha256)와 <code>rules</code> 를 넣는다. 봉인 카드에는 <b>\"이 봉인이 보장하지 않는 것\"</b> 절을 반드시 둔다 — 봉인은 \"규칙이 안 바뀌었다\"만 보장하지 <b>그 규칙이 옳다</b>고는 말하지 않는다.<br>⚠ 봉인된 파일의 문구를 나중에 고치면 <b>봉인이 철회된 문구를 보증하는 꼴</b>이 된다. 문구를 고쳐야 하면 봉인을 새로 판다.",
 "ours":"<a href=\"/files?q=cascade_seal_v2\">cascade_seal_v2_2026_09_08.json</a> 이 현행 봉인이고, <code>⚠_미정_규칙</code> · <code>⚠_부분미정_규칙</code> · <code>⛔_이_봉인이_보장하지_않는_것</code> 세 칸을 비워 두지 않는다 — 못 정한 것을 정한 척하지 않는 게 봉인의 값어치다.<br>사전등록 선례: <a href=\"/files?q=prereg\">prereg_d_rel_2026_08_28</a> · <code>b2o3_uma_vs_dft_force_prereg_2026_09_08</code> · <code>sdcp_polaron_pilot_prereg_S0_2026_08_31</code>.<br>⚠ 비준 지문(<code>ratification.content_digest</code>)이 박힌 사전등록·마감 카드는 본문을 고치지 않는다. 고치면 지문이 깨지고, 깨진 지문은 \"승인 뒤 본문이 바뀌었다\"로 화면에 뜬다."},
]
