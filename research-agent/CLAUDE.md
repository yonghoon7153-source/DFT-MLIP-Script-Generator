# research-agent — 논문 자동비서 (하위 폴더 지침)

> 이 파일은 **`research-agent/` 폴더 전용 지침**이다. 2026-09-09 에 루트 `CLAUDE.md` 에서
> 떼어 왔다 — 거기 있는 동안 상대경로 10개(`config/research_profile.md` ·
> `data/papers.sqlite` · `research_agent/vault.py` · `prompts/style_guide.md` ·
> `SETUP_CLAUDE_CODE.md` · `VERSION` · `CHANGELOG.md` · `config/agent.yaml` ·
> `data/papers.jsonl` · `data/analysis/pending`)가 **전부 repo 루트에서 안 풀렸고**,
> "이 폴더는 논문 에이전트다" 라는 첫 문장이 **전고체전지 계산 캠페인 repo** 를 가리켰다.
> 이제 경로는 **이 폴더 기준**으로 읽는다.
>
> ⚠ **이름 충돌 주의** — 루트 CLAUDE.md 의 *"논문 에이전트 요청 = litdb-curator 서브에이전트"*
> 는 **다른 것**이다. 구분:
> · **litdb 큐레이션**(digest·INDEX·comparison 갱신) = `.claude/agents/litdb-curator.md`
> · **메일·큐·vault**(이 폴더) = research-agent · `.claude/agents/paper-analyst.md`
>
> repo 전체 규율(데이터 규율·용어·마감·kb 위키 등)은 루트 `CLAUDE.md` 가 정본이고,
> 이 파일은 그것을 **덮어쓰지 않는다**.
