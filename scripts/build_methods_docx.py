...
METHODS_FULL = [
    ...
    ('Conductivity of the carbon network.', 'The coefficient assigned to the VGCF phase (100 S cm⁻¹) is a frozen, uncalibrated legacy voxel-network coefficient, not a fibre material constant and not a value derived from any measurement reported here. It was introduced as an explicit order-of-magnitude placeholder, taken from the low end of a range cited at the time as the literature band for graphitic fibre (10²–10³ S cm⁻¹); that band was subsequently found to describe compacted powder rather than single filaments, VGCF-H having a single-filament resistivity of 1 × 10⁻⁴ Ω cm (10⁴ S cm⁻¹). The coefficient therefore coincides with the powder regime rather than having been selected against it, and it has not been recalibrated since. It is retained so that the results presented in this work remain reproducible. Voxelisation fuses touching fibres into shared cells and therefore does not resolve fibre–fibre contact resistance, which is one of several contributions separating single-filament (≈ 10⁴ S cm⁻¹) from compressed-powder (≈ 83 S cm⁻¹) measurements of VGCF-H; packing fraction, orientation, network tortuosity, contact number, compaction pressure and the measurement configuration also enter the powder value. The coefficient is rescaled with voxel size so that the one-voxel-thick tube carries the axial conductance of a 0.15 μm fibre, σ_eff = σ·πd²/(4h²), giving 78.5 S cm⁻¹ at h = 0.15 μm. Because this coefficient was not independently calibrated, the conductivities and ratios below are protocol responses under a stated closure rather than material-level estimates.'),
    ...
    ('Limitations.', 'The absolute conductivities have not been calibrated against a composition-matched measurement and should be read as the output of an idealised bulk model: the solver places no contact resistance at any interface — between active particles, between active material and carbon, or at the current collector — so it does not reproduce the quantity a two-terminal DC-polarisation measurement returns. The ratio is not grid-converged: refining the voxel edge increases it monotonically without following a power law, so the reported gain is larger at finer voxels over the refinement interval examined; neither a continuum extrapolation nor a global bound is established. Explicitly restoring the additive contacts that voxelisation drops recovers only about a fifth of that grid dependence — measured under the binder-omitted convention only, and not transferable to the exact-zero convention. The magnitude is also conditional on the carbon conductivity being treated as an effective network constant: at the single-filament value, a hundredfold higher, the ordering reverses. That upper arm is not better physics but a doubly idealised sensitivity at one origin phase, assuming perfect fibre–fibre contact on top of equipotential fibres; a single scalar can absorb part of the missing resistance numerically but is not identified as a fibre–fibre contact parameter. Three further limits apply to the beds themselves. They are more compacted than the experimental porosity anchor. The compaction was not quasi-static: the platen advanced at 0.27 of the dilatational wave speed against an internal limit of 0.01, so the bed state carries a rate contribution. Both electrodes were compacted at the same platen speed, which makes the inputs like-for-like but does not imply that the rate contribution cancels in the ratio — equal driving speed is not equal dynamic response for two different compositions. The reported ratio is therefore conditional on this high-rate compaction protocol, and the absolute geometric quantities are a terminal wall separation and a high-rate simulation-geometry diagnostic rather than quasi-static converged values. Finally, the specimen provenance of the SDCP conductivity (250 S cm⁻¹) is not established.'),
]
