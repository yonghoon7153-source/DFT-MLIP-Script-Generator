...
def provenance(extra=None, paths=('scripts/',)):
    ...
    그 **입력 파일 경로**를 함께 넘겨라 — 그래야 "그 데이터가 커밋된 것인가" 까지 봉인된다.

    ⚠⚠ **범위에 출력 디렉터리를 넣지 마라.**  첫 실측을 쓰는 순간 그것이 dirty 로 잡혀 같은
    배치의 다음 실측이 **자기 게이트에 걸린다**.  실제로 걸렸다 (2026-09-07, `docs/data/` 를
    통째로 범위에 넣었더니 2건 중 2번째가 거부됨).  dirty 범위는 **입력**이지 산출물이 아니다.
    """
