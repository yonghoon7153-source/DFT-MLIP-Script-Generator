...
#
#   bash scripts/setup-wsl.sh            # venv + 의존성 + alias li2s
...
#   2. .venv 생성, webapp/requirements.txt + pymupdf(그림 크로핑) + anthropic(/chat) 설치
#   3. ~/.bashrc (있으면 ~/.zshrc 도) 에  alias li2s='<이 저장소>/webapp/li2s.sh'  추가
#   4. wslu(wslview) 가 있는지 알려 준다 — 없어도 li2s 는 powershell.exe/cmd.exe 로 브라우저를 연다
set -uo pipefail
...
"$ROOT/.venv/bin/pip" install --quiet -r "$ROOT/webapp/requirements.txt" pymupdf pillow anthropic python-docx
sha256sum "$ROOT/webapp/requirements.txt" | cut -d' ' -f1 > "$ROOT/.venv/.li2s.reqs.sha256"
"$ROOT/.venv/bin/python" -c "import flask, markdown, yaml, fitz, anthropic; print('· flask', flask.__version__, '· markdown', markdown.__version__, '· pymupdf', fitz.__doc__.split()[1] if fitz.__doc__ else '', '· anthropic', anthropic.__version__)"

echo "== 3. alias =="
if [[ $NO_ALIAS -eq 0 ]]; then
  bash "$ROOT/scripts/install-alias.sh"
else
  echo "· 건너뜀 (--no-alias)"
fi
...
echo
echo "✓ 끝. 새 셸을 열거나  source ~/.bashrc  한 뒤:   li2s"
