---
description: 단일 페이지를 raw 원문(digest·세션 기록)과 대조 검증하고 verificationStatus 를 기록
---
...
1. 대상 페이지를 읽고, 검증 가능한 주장(claim)들을 추린다. 전압마다 기준전극이 있는지, 비용량/면적용량이 구분됐는지, NCA721/논문 조성
   표기가 원문 그대로인지도 본다 (절대 규칙).
2. frontmatter 의 `sources` 에 적힌 raw 파일(들)을 열어 각 주장을 원문과 대조한다. 논문 노트는 `paper:` 의 값 하나하나를 digest 의 `[인쇄]`
   좌표와 대조한다 — 근거 없는 값은 null 로 되돌린다.
...
**금지**: `explored` 필드는 절대 건드리지 않는다 — 사람이 읽었는가는 사람만 바꾼다. raw 는 수정하지 않는다.
