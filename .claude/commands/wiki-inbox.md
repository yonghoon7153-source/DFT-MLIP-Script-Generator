...
4. **처리**: PDF/논문(.docx SI 포함)은 `/paper` 로 넘긴다. 그 밖의 항목은 `/wiki-ingest` 절차를 그대로 수행한다 — raw 저장(frontmatter: source_url/wiki-ingested/sha256) → 중복 확인 → 위키 컴파일(`explored: false`, `verificationStatus: unverified`) → `wiki/index.md`/`wiki/log.md` 갱신.
