...
5. **논문이면 (DOI/arXiv/저널)**: 이 저장소의 기본은 **`/paper`** (논문 에이전트 — 절별 전문 digest + 그림 + `compare:`)다. 짧은 글·리뷰 요약만 여기서 일반 컴파일한다. 프로젝트가 이 논문의 수치·정의를 verbatim 반복 참조할 것 같으면 **Paper Ingest Mode 를 사용자에게 제안**한다 (`wiki/guides/paper-ingest-mode.md`) — 승인 전에는 실행 금지, 승인 후에도 필요한 좌표만.
