...
왜 다시 만드나 — 이미 치른 하자 5개를 구조적으로 막기 위해서다
  ...
  ④ Li 추출을 결합으로 오독   → freeze 0.6 의 -1.465 eV 가 그거였다. 깨끗한 슬랩 대비
     표면 Li 변위·배위 이전을 재서 **격리**한다 — 격리 근거는 **기하**다(변위·배위수 차),
     에너지 부호가 아니다.
     ⛔ 2026-08-28 철회 — 옛 판은 이 자리에 "VASP dE_extract = +0.336 eV 로 반증됨" 을
       근거로 적었다. 회신 P 2번 P0 가 그 부호를 **철회**하고 `citable: no` 로 강등했다
       (db/properties/sdcp_doped_closed_2026_08_28.json · citation_hazards
        sdcp_phaseB_dftu_v1 = BLOCKED "아무것도 인용하지 않는다").
       두 labeled endpoint 의 총자화가 2.378 / 0.518 μB 로 **자기상태가 다르고**
       basin 최소 동등성이 미입증이라, 그 단일점 차(≈ +0.34 eV)는 열역학 부호가 아니다.
       ★ 그래서 이 엔진의 추출 격리는 그 숫자에 기대지 않는다.
