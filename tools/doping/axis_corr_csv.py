...
#: ⛔⛔ **삭제됨 (2026-09-08 비준).** 여기에 `D_REL_GATE = 0.90` 이 있었다.
#:   근거는 Deng 2026 Angew 의 "σ 손실 7 % 허용 / 31 % 실패" 였는데, 비준 카드
#:   `db/properties/cascade_d_rel_estimand_2026_09_08.json` §1 이 그것을 명시적으로
#:   지웠다: *"Deng 의 7%/31% 는 표면 코팅 LPSC 의 상온 pellet EIS 다. 벌크 25% 치환
#:   문턱으로 옮길 외적 타당성이 없다 — 물리 게이트가 아니라 **임의 운영 cutoff** 였다."*
#:   그리고 이 캠페인의 보고량은 **tracer D 비**이지 전도도가 아니다(§1: Li 개수가
#:   host 24 대비 18–28 이라 D* 비가 같아도 σ 비가 같지 않다).
#:   ★ 되살리지 말 것. 되살리면 selftest 가 **실패**한다 (아래 [09-08비준·음성]).
#:   ★ 상수를 지웠으므로 `from … import D_REL_GATE` 는 ImportError 로 즉시 드러난다 —
#:     조용히 옛 문턱으로 도는 경로를 남기지 않는 것이 목적이다.
#: 비준 카드가 지운 문구 — 산출물 어디에도 나오면 안 된다 (selftest 가 이걸로 잡는다).
RETRACTED_PREREG_PHRASES = (
    "D_rel ≥ 0.90", "D_rel >= 0.90", "0.90 게이트", "D_REL_GATE",
    "|ρ| < 0.2", "|rho| < 0.2", "이동도 기술자는 D 를 예측하지 못한다",
...
def preregister(designs, targets, predictor="li_mobility_score", top_k=10):
    ...
    ★ 2026-09-08 — 성공기준을 **비준 카드로 교체했다**
      `db/properties/cascade_d_rel_estimand_2026_09_08.json`
      (D-2026-09-08-cascade-d-rel-estimand · status ratified · 결과 보기 전 비준).
      옛 기준 두 줄이 그 카드에서 **삭제**됐는데 이 함수가 계속 찍고 있었다:
        · `D_rel ≥ 0.90` 게이트 → §1 "임의 운영 cutoff … 외적 타당성 없음"
        · `|ρ| < 0.2 ⇒ 축 근거 없음` → §3 "n=39 에서 진짜 ρ=0 이어도 Fisher 95% 구간이
          약 ±0.31 — 작은 점추정은 무효 입증이 아니라 **정밀도 부족으로 판정 불가**"
      사전등록의 요점은 "측정 전에 박은 기준" 인데, 그 기준이 이미 철회된 것이면
      기록의 의미가 반대로 뒤집힌다(철회된 규칙을 사전등록이 보증하는 꼴).

    ⛔ 못 하는 것
      · **채점하지 않는다.** 라벨이 없다. 이 함수는 기록만 남긴다.
      · 예측기를 고르지 않는다 — 부르는 쪽이 정하고, 그 이름이 기록에 남는다.
      · 순위가 맞아도 **인과가 아니다.** 이동도 기술자와 D 가 같이 움직인다는 것뿐이다.
      · **CI 를 계산하지 않는다.** §3 이 요구하는 Fisher CI·블록 부트스트랩·유효표본수는
        채점 단계(라벨이 들어온 뒤)의 일이다 — 여기서는 그 규칙을 **적어만** 둔다.
      · `top_k` 는 이제 보고 구간 표시일 뿐 **판정에 안 쓴다** (게이트가 삭제됐다).
    """
    by_name = {}
    for d in designs:
        nm = d.get("name")
    ...
    return {
        ...
        "보고량": "D_rel(design, host) ≡ D*(design)/D*(host) · 600 K · MSD 창 2–50 ps · "
                 "cell-conditioned (비준 카드 §1)",
        "예측": "이 순위가 곧 측정될 D_rel_vs_host 순위와 **양의 상관**을 가진다",
        "판정규칙_출처": {
            "card": "db/properties/cascade_d_rel_estimand_2026_09_08.json",
            "decision": "D-2026-09-08-cascade-d-rel-estimand",
            "state": "ratified (2026-09-08, 결과 보기 전)"},
        "성공기준_사전확정": {
            "0_보고": "ρ̂ 만 쓰지 않는다 — **ρ̂ + Fisher 95% CI + 유효 표본수**를 같이 "
                     "보고한다 (§3 대체_규칙)",
            "1_지지": "CI **전체**가 사전 고정 문턱 위",
            "2_반증": "CI **전체**가 사전 고정 등가영역 안. ⚠ ±0.2 등가에는 이상적 "
                     "조건에서도 n ≥ 69 가 필요하다 — n=39 로는 **반증을 못 한다**고 "
                     "결과 보기 전에 선언한다",
            "3_판정불가": "CI 가 걸치면 '이 표본으로는 판정 불가(정밀도 부족)'라고 적는다. "
                        "**'효과 없음' 으로 읽지 않는다**",
            "4_클러스터": "39개는 30 dopant · 10여 구조환경에 묶여 독립 표본이 아니다 "
                        "(상위 10 중 7이 같은 환경군) — 환경 단위 블록 부트스트랩 또는 "
                        "cluster-robust CI 를 쓰고 유효 표본수를 39 가 아니라 그 값으로 적는다",
            "5_검정력_사전고지": "n=39 에서 진짜 ρ=0.35 여도 통과확률 약 50%, 80% power 에는 "
                             "ρ≈0.47 이 필요하다",
            "⛔_삭제된_기준": [
                "`D_rel ≥ 0.90` 게이트 — 임의 운영 cutoff 였다 (Deng 의 7%/31% 는 표면 코팅 "
                "LPSC 의 상온 pellet EIS 라 벌크 25% 치환 문턱으로 옮길 외적 타당성이 없다)",
                "`|ρ| < 0.2 ⇒ 이동도 축 근거 없음` — 작은 점추정은 무효 입증이 아니다",
                "300 K σ 게이트 · 2온도 외삽 — 이 캠페인은 그것을 재지 않는다"],
            "⛔": "기준을 **측정 뒤에 고치지 않는다.** 고치면 이 기록의 의미가 사라진다. "
                 "위 삭제는 측정 **전**(2026-09-08 비준)에 이뤄진 것이다"},
        "보고구간_top_k": top_k,
        "ranking": scored,
    }
...
def _selftest():
    ...
    # ⛔ 2026-09-08 — 여기 있던 단언은 `0.5 < D_REL_GATE < 1.0` 이었다. 즉 **삭제된
    #   게이트가 살아 있어야 통과**하는 시험이라, 비준 카드를 따르면 selftest 가 깨졌다.
    #   시험이 철회된 규칙을 지키고 있었던 셈이라 방향을 뒤집는다.
    say("D_REL_GATE" not in globals(),
        "[09-08비준·음성] 삭제된 0.90 게이트 상수가 되살아나면 실패한다")
    ...
    say(all(x in k for x in ("0_보고", "1_지지", "2_반증", "3_판정불가")),
        "[사전등록·음성] 성공·반증·판정불가 기준이 **측정 전에** 박혀 있다")
    say("3_판정불가" in k and "효과 없음" in k["3_판정불가"],
        "[사전등록] **판정불가**를 따로 적는다 — 없으면 정밀도 부족이 '효과 없음' 으로 읽힌다")
    say(pr.get("frozen_at") and pr.get("predictor") == "li_mobility_score",
        "[사전등록] 언제·무엇으로 얼렸는지 남는다")
    # ★★★ 09-08 비준 카드 정합 — **음성 방향이 핵심이다** ★★★
    #   옛 selftest 는 "0.90 게이트·|ρ|<0.2 문구가 **있어야** 통과" 였다. 비준 카드가
    #   그 둘을 삭제했으므로 이제는 **나오면 실패**다. 방향을 안 뒤집으면 시험이
    #   철회된 규칙을 지키는 문지기가 된다 (이 파일이 실제로 그 상태였다).
    #   ⚠ 스캔에서 `⛔_삭제된_기준` 은 뺀다 — 거기 적힌 문구는 **이력**이지 판정 규칙이
    #     아니다. 이력까지 지우면 "왜 지웠나" 가 사라진다 (접기는 되고 삭제는 안 된다).
    import json as _js
    _op = {kk: vv for kk, vv in (pr.get("성공기준_사전확정") or {}).items()
           if kk != "⛔_삭제된_기준"}
    _blob = _js.dumps({**pr, "성공기준_사전확정": _op}, ensure_ascii=False)
    _hit = [p for p in RETRACTED_PREREG_PHRASES if p in _blob]
    say(not _hit, f"[09-08비준·음성] 삭제된 판정 문구가 사전등록 산출물에 없다 (발견: {_hit})")
    say(any("0.90" in s for s in (pr.get("성공기준_사전확정") or {})
            .get("⛔_삭제된_기준", [])),
        "[09-08비준] …단 **이력 절에는 남아 있어야** 한다 (조용한 삭제 금지)")
    say("판정규칙_출처" in pr and "cascade_d_rel_estimand_2026_09_08" in
        (pr.get("판정규칙_출처") or {}).get("card", ""),
        "[09-08비준] 판정 규칙의 출처(비준 카드)를 산출물에 적는다")
    say("Fisher" in (k.get("0_보고") or "") and "유효 표본수" in (k.get("0_보고") or ""),
        "[09-08비준] ρ̂ 단독 보고를 금지하고 CI·유효표본수를 요구한다 (§3)")
    say("n ≥ 69" in (k.get("2_반증") or "") or "69" in (k.get("2_반증") or ""),
        "[09-08비준] n=39 로는 **반증을 못 한다**를 결과 보기 전에 선언한다")
