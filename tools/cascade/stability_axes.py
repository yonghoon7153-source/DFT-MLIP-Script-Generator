...
**T10 E_hull 합성가능성** — Lee 2024 Table 1 이 `E_hull < 50 meV/atom` 을 합성가능성 기술자로 쓴다.
  ...
  ⛔⛔ 2026-09-09 — **이 경고가 여기 있었는데도 우리가 지나쳤다.** 이 표를 도핑체 데이터로
     읽고 배치 축소안을 세웠다가 회신 BJ 에서 통째로 NO-GO 를 받았다. 그래서 이제
     산출 CSV 머리에 `substitution_mode: separate_phase` 와 **보존명**을 박는다 —
     "별도 상의 벌크 상평형 지표 · 저장소/경쟁상 후보 목록".
     ⚠ 그리고 **별도 상 = 코팅** 도 아니다. 코팅인지 입계 첨가상인지는 이 표가 안 정한다.
     ⚠ **μ_M 으로 전환 금지** — μ_O 를 안 정하면 μ_M 이 하나로 안 정해진다 (회신 BK §4).
