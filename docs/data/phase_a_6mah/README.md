...
⚠ **SE 필터 규칙**: 중심이 `0 ≤ z ≤ 0.113419` 인 것만.  37 개(0.023 %)가 빠졌고 전부 SE 다
(바닥 벽 아래로 샌 입자).  AM 은 하나도 안 빠졌다.  porosity 는 이 규칙과 무관하게 14.28 %
(전부 포함해도 같다) — 즉 **이 필터는 결론에 영향이 없다**.
...
**① `d_h/dx` 게이트 = 통과** (실측): `d_h = 0.9810 µm` · φ_SE_local 0.6723 ·
**최소 통과 n_grid = 179** ⇒ 계획 격자 288 에서 **5.65**.  STEP2 결과에 하한 라벨 불필요.
...
|---|---|---|---|---|---|
| VGCF 1 + PTFE 1 | 26,967 | 2,206 | 1.961 | 80.57 | 17.43 |
| VGCF 2 + PTFE 1 | 54,490 | 2,229 | 3.962 | 79.75 | 17.25 |
| VGCF 3 + PTFE 1 | 82,587 | 2,252 | 6.005 | 78.92 | 17.08 |
| VGCF 4 + PTFE 1 | 111,275 | 2,276 | 8.091 | 78.10 | 16.90 |
