...
porosity 14.321 %                   ← **정본**.  킷 mpm_input.json 도 이 값 (0.1432)
...
### ⚠⚠ 재하율 — 킷은 **`--platen-mach 0.03` 고정**이다 (2026-09-07, Codex R9)

초판 킷은 기하 규칙 `vmax = 0.008·H` 를 썼는데 그것이 속도를 **침대 높이에 비례**시킨다.
`dilate_z` 가 조성마다 다르므로(1.0214 → 1.1085) **Mach 가 조성과 함께 오른다**:

```
             dilate_z   Mach (실측)      Codex 예측
VGCF 1 wt%    1.0214     0.4275          0.42745    ✓ 4자리 일치
VGCF 2 wt%    1.0519     0.4393          0.43928    ✓
VGCF 3 wt%    1.0798        —            0.45010
VGCF 4 wt%    1.1085        —            0.46123
                                         ⇒ +7.9 % 단조 증가
```

⇒ **공통모드가 아니라 조성축에 걸린 수치적 재하율 축**이다.  그래서 킷은 이제 Mach 를
**0.03 으로 고정**한다.  ⛔ 기하 규칙으로 되돌리는 개정은 허용되지 않는다 (Codex R9).

⚠ **`--frames` 는 하드 캡**이다 (`range(args.frames)`) — 초과하면 `not_converged_frame_budget`
으로 죽고 **숫자가 하나도 안 나온다**.  Mach 를 낮추면 프레임당 하강폭이 그만큼 줄므로 같은
배수로 늘려야 한다.  이 킷들은 **2500** 이고, 생성기가 짝이 안 맞으면 **거부**한다.

⚠ **`MPM_QUASISTATIC=1` 은 이제 쓰지 말 것** — 그 분기는 `--platen-mach 0.01 --frames 1500` 인데
0.01 에는 7500 프레임이 필요하다.  쓰면 프레임 예산에서 죽는다.
...
⚠⚠ **W1·W2 실측(2026-09-07)은 exploratory pilot 이다 — 본 판정에서 제외한다.**
기하 규칙(Mach 0.4275 / 0.4393)으로 돌았고 등록 규약(0.03)이 아니다.  파이프라인 생존과
Mach 의 조성 의존을 확인한 값으로만 쓴다.
...
**③ ⛔ 각도 위험 선별 = 무효** (Codex R9 Q3 — 아래 값을 **인용하지 말 것**):

세 가지가 동시에 어긋난다.
· 도구가 **무첨가 강체 CSV** 만 읽는다 — STEP2 의 `se_dump/phase/fibre` 를 안 읽으므로 실제
  조성 침대가 아니다.
· 생산은 **x/y periodic** 인데 KDTree 에 periodic image 가 없다 (독립 재계산: p05 0.39985 →
  0.46700, K 8 → 7).
· `angular_risk.json` 자체가 **`code_dirty = true`** 다 — 내 봉인 게이트가 인용을 금지한다.
⇒ 네 조성의 **post-STEP2 상태**에서 **periodic-aware** 로 다시 재야 한다.  비교에는 조성별
다른 K 가 아니라 `K_common = max_j ceil(3/p05_j)` 이상의 최소 ladder 값을 공통 적용한다.

아래는 **폐기된 초판 측정**이다 (기록용):
