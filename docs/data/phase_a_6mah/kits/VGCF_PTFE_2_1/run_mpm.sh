...
if [ -z "${MPM_DETACHED:-}" ]; then
  ...
  export RUN_DIR="$KIT/run_VGCF2_PTFE1_$(date +%Y%m%d_%H%M%S)_$$"
  mkdir -p "$RUN_DIR"
...
python3 "$SCR/mpm3d_compaction.py" \
  ...
  --add-recipe "VGCF:PTFE=2:1" --add-l-cv 0.4 --mixing thinky --coh-ptfe 0.10 --binder-opt-wt 1.5 --add-rng-per-phase --fibre-buckle --fibre-align 0.661 --dilate-z 1.0519 --save-phase phase.npy --save-fibre fibre.npy --save-fibre-dia fibre_dia.npy "${FRAC[@]}" \
