...
if [ -z "${MPM_DETACHED:-}" ]; then
  ...
  export RUN_DIR="$KIT/run_VGCF3_PTFE1_$(date +%Y%m%d_%H%M%S)_$$"
  mkdir -p "$RUN_DIR"
...
python3 "$SCR/mpm3d_compaction.py" \
  ...
  --add-recipe "VGCF:PTFE=3:1" --add-l-cv 0.4 --mixing thinky --coh-ptfe 0.10 --binder-opt-wt 1.5 --add-rng-per-phase --fibre-buckle --fibre-align 0.666 --dilate-z 1.0798 --save-phase phase.npy --save-fibre fibre.npy --save-fibre-dia fibre_dia.npy "${FRAC[@]}" \
...
python3 "$SCR/mpm_webapp_payload.py" \
  ...
  --n-vox 192 --tri-step 4 --smooth 1.5 --target-porosity 0.1589 --eps se_dump_eps.npy --dilate-z 1.0798 \
