...
**(c)** n = 6, spin per thiophene ring along the chain (preliminary: the terminal rings 0 and 5 still
include one α-H each; sum 79.5 %). The spin peaks on rings 3–5.
...
- **(b) n 에 따른 이동.** 같은 분할로 n=1 → 6 을 나열. SO₃ 65 → 62 → 55/42 → 7.7 로 줄고, 고리는
  ...
  넘는 것이 7월의 크로스오버.
- **(c) 고리별.** n=6 의 79.7 이 어느 고리에 있는지. ring4 23.3 > ring5 19.8 > ring3 15.9 순으로
  사슬 한쪽(3–5번 고리)에 몰려 있다. ⚠ 예비값: 옛 groups.json 에 symbols 가 없어 말단 고리(0·5)에
  α-H 가 하나씩 섞였다(합 79.5 = strict). 순서는 −0.2 %p 오염으로 안 바뀌지만 수치 인용은
  gabia 재분석(`nseries_n6.py --analyze`, 재계산 없음) 뒤에.
