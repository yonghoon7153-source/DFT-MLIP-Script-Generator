...
| **CL-41** | live | 세 점 완주 (0.15/0.125/0.115).  ⛔ 세 비 값은 **인용 금지** (p1 plate rule, 원장에만).  **멱법칙 불성립** (증분비 1.773 < 이론 최소 2.187) ⇒ Richardson 무의미, 보고값은 전부 **하한** |
| **CL-45** | live · `BOTH_REJECTED` | σ-치환 감소율 **30.38 %** (vox 0.4).  비례계수 0.912(0.15) → 0.763(0.4) = 수확체감.  CL-43 의 동행은 **인과 아님** |
| **CL-58** | live · measurement_record | 도핑 baseline (8팔, 3게이트 통과).  ⛔ σ_e 비·이득 값은 **인용 금지** (CL-33 계열 p1 산물, 원장에만).  이온은 −0.73 % |
