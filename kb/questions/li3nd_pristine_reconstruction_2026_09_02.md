...
> ⚠⚠ **2026-09-06 정정 — 이 문장은 너무 세다.** rattle 시드 넷을 다 보니 갈린다:
>
> | 시드 | r0 대비 | 상태 |
> |---|---:|---|
> | r0 (무변형) | — | 수렴, **bfgs 0 스텝** (대칭이 힘을 0으로 만든다) |
> | **r1** | **−6.9 meV** | **수렴** (4 스텝) — 흔들었다가 **원래 basin 으로 돌아왔다** |
> | r2 | −0.667 eV | step 42, **미완** |
> | r3 | −1.574 eV | step 50 nstep 소진, **미완** |
>
> **셋 중 하나는 안 무너진다.** 그러면 그 구조는 *"최소가 아니다"* 가 아니라
> **"국소최소이되 특정 방향으로 낮은 장벽을 갖는다"** 가 맞다.
> ⇒ 재구성은 **보편적이 아니라 방향 의존적**이다. NEB 끝점이 어느 방향으로
>   이완됐느냐에 따라 장벽 기준상태가 달라진다는 뜻이라, 결정 실험이 더 중요해졌다.
>
> ⚠ 아직 단정 못 하는 것: r1 이 **4 스텝만에** 끝났다(rattle 이 소프트모드를 안
> 건드렸을 수 있다) · r2·r3 는 **최종 에너지를 모른다** · r2 는 OOM 이라
> `from_scratch` 로 다시 걸려 이전 −0.667 eV 궤적은 `.out.000` 에만 있다.
...
- ⛔ **"pristine 3×3×3 은 최소가 아니다"** — 2026-09-06 정정. r1 이 −6.9 meV 로 돌아왔다.
  쓸 수 있는 것은 *"rattle 방향에 따라 갈린다 — 셋 중 하나는 제자리, 둘은 0.7~1.6 eV 하강(미완)"* 이다.
- ⛔ r1 하나로 **"안정하다"** 도 쓰지 않는다. 4 스텝 수렴이라 조기수렴 가능성이 남는다.
