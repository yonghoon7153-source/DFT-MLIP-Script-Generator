...
> - **σ 절대값은 CLAUDE.md 규율상 인용 금지**(Nernst-Einstein `H_R=1` 가정 · RT 외삽 보고 불가).
>   ⚠ 2026-09-07: 사유에서 **"상한"** 이라는 말을 뺐다 — `H_R<1` 이면 NE 는 오히려 과소다(§4 정정).
>   비율도 멀티시드 판정만.
>
...
- **절대 σ 는 인용하지 않는다** → **Ea·상대비로 인용**.

> [!warning] ⛔ 2026-09-07 정정 — 이 줄은 원래 *"절대 σ는 UMA가 3–5× 과대 … 절대값은 상한"* 이었다
> **(a) 근거가 불명이고 (b) 다른 문서와 부호가 어긋났다.**
> · `H_R ≡ D*/D_σ` 이고 우리 MSD 는 `D*` 를 준다 ⇒ `σ_NE = H_R × σ_true`.
>   `H_R < 1`(상관 이동)이면 NE 는 **과소**다 — Adeli 2019 실측 `H_R = 0.23` ⇒ `σ_true ≈ 4.3 σ_NE`.
>   따라서 **Haven 은 "상한" 의 근거가 못 된다** (`tools/modelc_v3/nernst_einstein_300K.py` L234 가
>   이미 그렇게 적고 있었다).
> · "3–5× 과대" 는 **σ_NE 를 냉간압축 펠릿 total EIS 와 비교한 값**으로 보인다. 그 비교엔
>   Haven(NE 과소)과 GB·기공(실험이 낮음)이 **반대 방향으로** 섞여 있어 한 배수로 못 뭉갠다.
> · **같은 양끼리** 대면 맞는다: 우리 `D*(300 K)` 외삽 1.02e-7 cm²/s vs Adeli ⁷Li PFG 실측
>   `D*` 1.01e-7 (단일시드 1.01배 · 3시드 2.29배).
> ⇒ 든든한 축은 **Ea 와 D\***, 흔들리는 것은 **D\* → σ 변환**이다.
>   고치려면 `H_R` 을 우리 궤적에서 직접 잰다 (`msd_diffusive_check.py --haven`).
>   ⚠ 그 전까지 σ 절대값 인용 금지는 **그대로 유지**한다 — 방향이 정해지지 않았기 때문이지
>   "상한이라서" 가 아니다.
