...
## Supplementary Note 2 (확정 · Note 1 형식)

> **Supplementary Note 2. Oligomer models of the oxidized SDCP chain**
>
...
> for three repeat units, oxidation at a terminal ring and at the internal ring was treated
> separately. The oxidized, deprotonated state was generated by removing a neutral hydrogen atom —
> one proton and one electron — from a sulfonic acid group, giving a charge-neutral species with one
> unpaired electron (charge 0, multiplicity 2). This represents self-doping, in which the proton is
> taken up by a base and the electron by an oxidant, leaving a hole on the chain and a sulfonate
> counter-anion; the oxidized state is therefore imposed by construction rather than predicted. All
> structures were optimized with ORCA 6.1.1 at the r²SCAN-3c level, which combines the r²SCAN
> meta-GGA functional with a def2-mTZVPP basis set, D4 dispersion, and a geometrical counterpoise
> correction, using the program's default convergence thresholds and applying no solvent model or
> symmetry constraint. The same protocol was used for every chain length.
>
> Löwdin spin populations were taken from the final SCF of each optimized structure; for six repeat
> units the total spin population is 1.000, as expected for a doublet. Throughout, the backbone
> denotes ring atoms only, with ring hydrogens and side-chain ether oxygens excluded. Including the
> ring hydrogens gives a backbone share of 79.5%, and further including the ether oxygens gives
> 91.6%, against 79.7% for the definition used here. For six repeat units the hydrogen was removed
> from the sulfonate on ring 3, determined from the optimized structure rather than from the run
> label, and the spin maximum falls on the adjacent ring. These models are isolated gas-phase
> oligomers, each obtained as a single self-consistent solution without a search over spin isomers or
> side-chain conformers, and the spin density is reported as such rather than equated with the hole
> charge density.

⚠ **Figure 1 참조를 없앴다.** 초안이 "the repeat unit defined in Figure 1" 로 가리켰는데, 원고
Figure 1 이 그 구조를 정의하는지 확인하지 않고 쓴 것이었다. 반복단위를 화학식으로 적고 합성
노트(Note 1)를 가리키는 쪽이 자체 완결이고, "side-chain ether oxygen" 이 EDOT 고리의
에틸렌디옥시 산소가 아니라 곁사슬의 그 산소라는 것도 화학식이 구분해 준다.
⏳ 곁사슬 `−CH₂O(CH₂)₄SO₃H` 는 Note 1 의 합성 경로에서 읽은 것 — hydroxymethyl-EDOT + butane
