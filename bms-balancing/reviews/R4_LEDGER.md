...
5. ~~GREEN + 반례 재실행~~ — 위 표
6. **사용자 기계 (U12)**: 네 루트에서 `eval` 을 돌려 `# scale_audit` 줄을 받는다 (Inf 표본 0 이면 그 자료에서 U1 동치) — 명령은 WORKING_STATE 에
