...
| 항목 | 값 |
|---|---|
| 커밋 | 이 파일이 들어 있는 커밋 (`git log -1`; 코드·문서의 마지막 변경은 R3 대응 커밋 `b0f1fbe` + 그 뒤의 `--precision` 오류 처리 소수정) |
...
| 우리 환경에서만 되는 것 | `scripts/ne_shape.py` 재실행 (R3-03 의 (d) 열 — **아래 §5 U11**) |
...
python3 -m pytest tests/ -q                       # 원자료 불필요. 62 passed · 1 skipped 기대 (skip = U11)
...
| 검사 | 결과 |
|---|---|
| `pytest tests/ -q` | 62 passed · 1 skipped (`test_r3_03_committed_ne_shape_csv_and_section_5_2_carry_the_headroom_not_the_box_claim`: 커밋된 CSV 에 (d) 열이 아직 없음) |
| `matlab/tests/run_all.sh` | 4·5 단계 통과 (Octave 없음) |
| R3 반례 5 스크립트 | 위 주석대로 — 넷은 수정 뒤 assertion 실패(닫힘), noise 는 재현(철회 근거) |
| R3-04 변이 감사 | 임시 사본 FINDINGS 변조 4 종 전부 회귀 실패, 원본 통과 |
...
| 03 "상자 안" | 닫힘 (코드+철회) · **실측 대기** | `ne_shape.gamma_headroom`: 합법 Δγ 구간 · 합법 γ 최대 변화(γ 격자 501 · x 400) · (a) 이상을 내는 γ_ref 최근접 합법 γ 증인(없으면 없음) → 출력 + CSV 열 6 (`legal_dgamma_neg/pos`, `gamma_family_max_mV`, `gamma_at_family_max`, `gamma_witness`, `gamma_witness_delta`). §5-2: γ_ref 0.2953 → [−0.295, +0.205], 외삽 100 ±0.066 · 200 ±0.106 · 300_0009 ±0.365(양쪽 밖) | `test_r3_03_ne_shape_*` (증인 ≈0.45 / 100 mV 오프셋에 증인 없음), `test_r3_03_committed_*` (CSV 재생성 후 활성) |
...
| **U11** | `out/ne_shape_GITT_Li.csv` 의 (d) 열 6 개 — 코드는 있으나 커밋된 CSV 는 옛 열 | **사용자 기계 재실행 대기** (`python3 scripts/ne_shape.py`); 그때까지 §5-2 는 "재실행 대기", 회귀 1 개 skip |

## 6. 리뷰어에게 — 질문

1. R3-01 대응이 충분한가 — "관측 유지 · 원인은 후보 가설" 로 낮춘 것 외에, 현재 자료(상태당 곡선 하나)로 할 수 있는 구분 시험이 있는가.
2. R3-03 의 (d): 증인을 "γ_ref 에 가장 가까운 합법 γ" 로 잡은 것과 격자(γ 501 · x 400)가 적절한가. 진폭의 존재 증인이 모양 일치가 아니라는 한정이 문서에서 떨어질 위험은 어디인가.
...
- `out/ne_shape_GITT_Li.csv` — R3 시점 그대로 (U11).
