...
검증 못 한 전제(주장 아님): R5-01 의 "Python format = MATLAB sprintf" — 반올림 타이·지수 자릿수는 MATLAB 판에 따라
다를 수 있고 그러면 **거짓 invalid**(fail-closed). 사용자 기계에서 `sprintf('%.2f',0.125)`·`sprintf('%.17g',1e-5)` 한 줄씩.
